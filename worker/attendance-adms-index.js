...
function promiseTimeout(promise,ms,message){
 ...
 return Promise.race([
  ...
  new Promise((_,reject)=>{timer=setTimeout(()=>reject(Object.assign(new Error(message||'انتهت مهلة العملية.'),{status:504,code:'WATCHDOG_TIMEOUT'})),ms)})
 ]);
}
async function reconcilePendingClockSyncArtifacts(env){
 const now=new Date(),nowIso=now.toISOString(),staleCutoff=new Date(now.getTime()-8*60*1000).toISOString(),recentCutoff=new Date(now.getTime()-4*60*1000).toISOString();
 const staleRuns=await rest(env,'attendance_watchdog_runs?source=eq.scheduled&status=eq.running&started_at=lt.'+enc(staleCutoff)+'&select=id,started_at&limit=100').catch(()=>[]);
 for(const run of staleRuns||[]){
  const startedMs=new Date(run.started_at||0).getTime(),duration=Number.isFinite(startedMs)?Math.max(0,now.getTime()-startedMs):null;
  await rest(env,'attendance_watchdog_runs?id=eq.'+enc(run.id),{method:'PATCH',body:{status:'failed',completed_at:nowIso,duration_ms:duration,error_text:'انتهت مهلة تشغيل المراقب قبل اكتماله؛ تم إغلاق التشغيل العالق تلقائيًا.',metadata:{auto_closed_stale_run:true}},prefer:'return=minimal'}).catch(()=>{});
 }
 const devices=await rest(env,'attendance_devices?status=eq.active&select=id,branch_id,metadata').catch(()=>[]);let resolvedNotifications=0,resolvedIncidents=0,pendingDevices=0;
 for(const device of devices||[]){
  const clock=device?.metadata?.clock_drift||{},sync=device?.metadata?.last_clock_sync||{},clockCheckedMs=new Date(clock.checked_at||0).getTime(),syncMs=new Date(sync.accepted_at||sync.time_callback_at||sync.requested_at||0).getTime(),pending=clock.confirmed===true&&sync.awaiting_verification===true&&['accepted','accepted_unverified','queued'].includes(txt(sync.status))&&Number.isFinite(syncMs)&&(!Number.isFinite(clockCheckedMs)||syncMs>clockCheckedMs);
  if(!pending)continue;pendingDevices+=1;
  const notes=await rest(env,'attendance_notifications?device_id=eq.'+enc(device.id)+'&active=eq.true&select=id,notification_key,category,metadata,status&limit=100').catch(()=>[]);
  for(const n of notes||[]){
   const isClock=txt(n.notification_key)==='device_clock_drift:'+device.id;
   const signals=Array.isArray(n?.metadata?.signals)?n.metadata.signals.map(txt).filter(Boolean):[],isWatch=txt(n.category)==='predictive'&&txt(n?.metadata?.type)==='watch'&&signals.length>0&&signals.every(s=>s==='درجة صحة الجهاز الحالية منخفضة.');
   if(!isClock&&!isWatch)continue;
   await rest(env,'attendance_notifications?id=eq.'+enc(n.id),{method:'PATCH',body:{active:false,status:'resolved',resolved_at:nowIso,resolved_by:'system:clock-sync',resolved_reason:'تم استلام مزامنة الساعة؛ بانتظار بصمة Live للتحقق، فلا يعتبر فرق الساعة السابق مشكلة نشطة.',updated_at:nowIso},prefer:'return=minimal'}).catch(()=>{});resolvedNotifications+=1;
   const incidents=await rest(env,'attendance_incidents?source_notification_id=eq.'+enc(n.id)+'&source_active=eq.true&select=id,status,started_at,resolution_due_at,resolved_at&limit=20').catch(()=>[]);
   for(const incident of incidents||[]){
    const startedMs=new Date(incident.started_at||0).getTime(),duration=Number.isFinite(startedMs)?Math.max(0,Math.round((now.getTime()-startedMs)/1000)):0;
    await rest(env,'attendance_incidents?id=eq.'+enc(incident.id),{method:'PATCH',body:{source_active:false,status:['resolved','closed'].includes(incident.status)?incident.status:'resolved',resolved_at:incident.resolved_at||nowIso,resolved_by:'system:clock-sync',resolution_reason:'تم استلام مزامنة الساعة؛ بانتظار التحقق من بصمة Live.',resolution_seconds:duration,updated_at:nowIso},prefer:'return=minimal'}).catch(()=>{});resolvedIncidents+=1;
   }
  }
 }
...
async function runAttendanceWatchdog(env,source='scheduled'){
 ...
 const runRows=await rest(env,'attendance_watchdog_runs',{method:'POST',body:{source,status:'running',runtime_mode:mode,started_at:startedIso,created_at:startedIso,metadata:{preflight}},prefer:'return=representation'}).catch(()=>[]);
 ...
 try{
  ...
  if(run?.id)await rest(env,'attendance_watchdog_runs?id=eq.'+enc(run.id),{method:'PATCH',body:{status:'failed',completed_at:completedAt.toISOString(),duration_ms:durationMs,error_text:message,metadata:{preflight,timeout:e?.code==='WATCHDOG_TIMEOUT'}},prefer:'return=minimal'}).catch(()=>{});
  throw e;
 }
}
...
async function attendanceState(env,me,url){
 ...
 const [devices,links,logs,unlinkedRows,employees,users,branches,shiftPeriods,scheduleVersions,deleteRequests,calendarRules,policies,policyVersions,mobileDevices,mobileEvents,mobileAttempts,correctionRequests]=await Promise.all([
  ...
  (()=>{const rg=saudiDayUtcRange(saudiTodayKey());return rest(env,'attendance_mobile_events?select=id,attendance_employee_id,staff_user_id,branch_id,mobile_device_id,event_type,occurred_at,accuracy_m,distance_from_site_m,inside_geofence,source,data_environment&data_environment=eq.'+enc(mode)+mobileDeviceFilter+'&occurred_at=gte.'+enc(rg.start)+'&occurred_at=lt.'+enc(rg.end)+'&order=occurred_at.desc&limit=2000').catch(()=>[])})(),
  (()=>{const rg=saudiDayUtcRange(saudiTodayKey());return rest(env,'attendance_mobile_attempts?select=id,attendance_employee_id,staff_user_id,branch_id,mobile_device_id,event_type,outcome,reason_code,reason_text,attempted_at,accuracy_m,distance_from_site_m,device_label,data_environment&data_environment=eq.'+enc(mode)+mobileDeviceFilter+'&attempted_at=gte.'+enc(rg.start)+'&attempted_at=lt.'+enc(rg.end)+'&order=attempted_at.desc&limit=2000').catch(()=>[])})(),
  rest(env,'attendance_correction_requests?select=id,attendance_employee_id,staff_user_id,branch_id,work_date,request_type,requested_event_type,proposed_at,source_kind,source_event_id,requested_reason,evidence,status,data_environment,requested_at,reviewed_at,reviewed_by,resolution_note,updated_at&data_environment=eq.'+enc(mode)+correctionFilter+'&order=requested_at.desc&limit=2000').catch(()=>[])
 ]);
...
async function attendanceReport(env,me,body){
 ...
 let overridePath='attendance_event_overrides?select=id,attendance_employee_id,branch_id,work_date,source_kind,source_event_id,override_action,replacement_manual_event_id,data_environment&active=eq.true&data_environment=eq.'+enc(mode)+'&work_date=gte.'+enc(txt(body.from_date))+'&work_date=lte.'+enc(txt(body.to_date||body.from_date));
 if(branchId)overridePath+='&branch_id=eq.'+enc(branchId);
 if(txt(body.attendance_employee_id))overridePath+='&attendance_employee_id=eq.'+enc(body.attendance_employee_id);
 const overrides=await rest(env,overridePath+'&limit=5000').catch(()=>[]),blockedSources=new Set((overrides||[]).map(x=>txt(x.source_kind)+'|'+txt(x.source_event_id)));
 let manualPath='attendance_manual_events?select=id,attendance_employee_id,staff_user_id,branch_id,work_date,event_type,occurred_at,source,correction_request_id,data_environment,metadata&data_environment=eq.'+enc(mode)+'&work_date=gte.'+enc(txt(body.from_date))+'&work_date=lte.'+enc(txt(body.to_date||body.from_date));
 if(branchId)manualPath+='&branch_id=eq.'+enc(branchId);
 if(txt(body.attendance_employee_id))manualPath+='&attendance_employee_id=eq.'+enc(body.attendance_employee_id);
 const manualRows=txt(body.device_id)?[]:await rest(env,manualPath+'&order=occurred_at.asc&limit=10000').catch(()=>[]);
 const visibleBiometricLogs=(biometricLogs||[]).filter(row=>!blockedSources.has('biometric|'+String(row.id))),visibleMobileRows=(mobileRows||[]).filter(row=>!blockedSources.has('mobile|'+String(row.id))),visibleManualRows=(manualRows||[]).filter(row=>!blockedSources.has('manual|'+String(row.id)));
 const employeeIds=[...new Set([...visibleMobileRows,...visibleManualRows].map(x=>x.attendance_employee_id).filter(Boolean))],eventEmployees=employeeIds.length?await rest(env,'attendance_employees?id=in.('+employeeIds.map(enc).join(',')+')&select=id,name,employee_code').catch(()=>[]):[],eventEmployeeMap=new Map((eventEmployees||[]).map(x=>[String(x.id),x]));
 const mobileLogs=visibleMobileRows.map(row=>({id:'mobile:'+row.id,device_id:null,serial_number:'MOBILE',branch_id:row.branch_id,device_pin:'MOBILE',attendance_employee_id:row.attendance_employee_id,staff_user_id:row.staff_user_id,employee_name:eventEmployeeMap.get(String(row.attendance_employee_id))?.name||null,occurred_at:row.occurred_at,device_time_raw:row.occurred_at,status_code:row.event_type==='check_in'?0:1,verify_code:null,work_code:'mobile:'+row.event_type,data_environment:row.data_environment,received_at:row.occurred_at,attendance_source:'mobile',mobile_event_type:row.event_type,metadata:{...(row.metadata||{}),accuracy_m:row.accuracy_m,distance_from_site_m:row.distance_from_site_m,inside_geofence:row.inside_geofence}}));
 const manualLogs=visibleManualRows.map(row=>({id:'manual:'+row.id,device_id:null,serial_number:'MANUAL',branch_id:row.branch_id,device_pin:'MANUAL',attendance_employee_id:row.attendance_employee_id,staff_user_id:row.staff_user_id,employee_name:eventEmployeeMap.get(String(row.attendance_employee_id))?.name||null,occurred_at:row.occurred_at,device_time_raw:row.occurred_at,status_code:row.event_type==='check_in'?0:1,verify_code:null,work_code:'manual:'+row.event_type,data_environment:row.data_environment,received_at:row.created_at||row.occurred_at,attendance_source:'manual',manual_event_type:row.event_type,metadata:{...(row.metadata||{}),correction_request_id:row.correction_request_id}}));
 const logs=[...visibleBiometricLogs,...mobileLogs,...manualLogs].sort((a,b)=>new Date(a.occurred_at)-new Date(b.occurred_at));
 ...
 return {ok:true,logs,calendar_rules:calendarRules,violation_decisions:violationDecisions,schedule_versions:scheduleVersions,month_closure:monthClosure,from_date:fromKey,to_date:toKey,environment:mode,branch_id:branchId||null,truncated:(Array.isArray(rawLogs)&&rawLogs.length>=10000)||(Array.isArray(mobileRows)&&mobileRows.length>=10000)||(Array.isArray(manualRows)&&manualRows.length>=10000),correction_overlay:{manual_events:visibleManualRows.length,overrides:(overrides||[]).length},clock_correction:{corrected_logs:clockCorrectedCount,device_ids:[...correctedDeviceIds],method:'confirmed_clock_history_matching_observed_drift'}};
}
...
async function recordMobileAttendanceAttempt(env,{employee,account,mode,mobileDevice=null,eventType=null,outcome='rejected',reasonCode,reasonText='',latitude=null,longitude=null,accuracy=null,distance=null,deviceLabel='',request=null}){
 try{
  return (await rest(env,'attendance_mobile_attempts',{method:'POST',body:{attendance_employee_id:employee.id,staff_user_id:account?.id||null,branch_id:employee.branch_id||null,mobile_device_id:mobileDevice?.id||null,event_type:['check_in','check_out'].includes(eventType)?eventType:null,outcome,reason_code:txt(reasonCode)||'unknown',reason_text:txt(reasonText)||null,attempted_at:new Date().toISOString(),latitude:Number.isFinite(Number(latitude))?Number(latitude):null,longitude:Number.isFinite(Number(longitude))?Number(longitude):null,accuracy_m:Number.isFinite(Number(accuracy))?Number(accuracy):null,distance_from_site_m:Number.isFinite(Number(distance))?Number(distance):null,device_label:txt(deviceLabel).slice(0,180)||null,metadata:{source_ip:request?clientIp(request):null},data_environment:mode},prefer:'return=representation'}))?.[0]||null;
 }catch{return null}
}
async function captureMobileAttendance(env,me,body,request){
 ...
 if(mobileDevice?.status==='revoked'){await recordMobileAttendanceAttempt(env,{employee,account,mode,mobileDevice,eventType,outcome:'rejected',reasonCode:'device_revoked',reasonText:'الجوال موقوف من تسجيل الحضور',deviceLabel,request});throw Object.assign(new Error('هذا الجوال موقوف من تسجيل الحضور. راجع الموارد البشرية.'),{status:403})}
 if(policy.mobile_require_trusted_device!==false&&mobileDevice?.status!=='approved'){await recordMobileAttendanceAttempt(env,{employee,account,mode,mobileDevice,eventType,outcome:'pending_device',reasonCode:'device_pending',reasonText:'الجوال يحتاج اعتماد الإدارة',deviceLabel,request});return {ok:true,pending_device:true,mobile_device_id:mobileDevice?.id||null,message:'تم التعرف على هذا الجوال، لكنه يحتاج اعتماد الإدارة قبل تسجيل الحضور.'}};
 ...
 if(policy.mobile_geofence_enabled!==false&&!policy.exceptions?.location_exempt){
  ...
  if(!Number.isFinite(lat)||lat<-90||lat>90||!Number.isFinite(lng)||lng<-180||lng>180){await recordMobileAttendanceAttempt(env,{employee,account,mode,mobileDevice,eventType,reasonCode:'location_unavailable',reasonText:'تعذر قراءة الموقع الحالي',deviceLabel,request});throw Object.assign(new Error('تعذر قراءة موقعك الحالي بشكل صحيح.'),{status:400})}
  if(!Number.isFinite(accuracy)||accuracy<0){await recordMobileAttendanceAttempt(env,{employee,account,mode,mobileDevice,eventType,reasonCode:'accuracy_unavailable',reasonText:'دقة الموقع غير متاحة',latitude:lat,longitude:lng,deviceLabel,request});throw Object.assign(new Error('دقة الموقع غير متاحة. فعّل الموقع الدقيق وحاول مرة أخرى.'),{status:400})}
  const maxAccuracy=Math.max(10,Number(policy.mobile_max_accuracy_m||120));
  if(accuracy>maxAccuracy){await recordMobileAttendanceAttempt(env,{employee,account,mode,mobileDevice,eventType,reasonCode:'accuracy_too_low',reasonText:'دقة GPS أضعف من الحد المسموح',latitude:lat,longitude:lng,accuracy,deviceLabel,request});throw Object.assign(new Error('دقة الموقع ضعيفة ('+Math.round(accuracy)+'م). انتظر تحسن GPS وحاول مرة أخرى.'),{status:409})}
  const siteLat=Number(policy.mobile_location_lat),siteLng=Number(policy.mobile_location_lng);
  if(!Number.isFinite(siteLat)||!Number.isFinite(siteLng)){await recordMobileAttendanceAttempt(env,{employee,account,mode,mobileDevice,eventType,reasonCode:'branch_location_missing',reasonText:'موقع الفرع غير مضبوط',latitude:lat,longitude:lng,accuracy,deviceLabel,request});throw Object.assign(new Error('موقع الفرع غير مضبوط في سياسة الحضور. راجع الإدارة.'),{status:409})}
  ...
  if(!inside){await recordMobileAttendanceAttempt(env,{employee,account,mode,mobileDevice,eventType,reasonCode:'outside_geofence',reasonText:'خارج نطاق مقر العمل',latitude:lat,longitude:lng,accuracy,distance,deviceLabel,request});throw Object.assign(new Error('أنت خارج نطاق مقر العمل بحوالي '+Math.round(distance)+' متر.'),{status:403})}
...
async function previewEmployeeAttendancePolicy(env,me,body){
 if(!canView(me))throw Object.assign(new Error('لا توجد صلاحية لعرض سياسة الحضور.'),{status:403});
 const employee=await scopedEmployee(env,me,txt(body.attendance_employee_id));
 if(!employee)throw Object.assign(new Error('موظف الحضور غير موجود أو خارج نطاق الفرع.'),{status:404});
 const day=txt(body.work_date)||saudiTodayKey();
 if(!/^\d{4}-\d{2}-\d{2}$/.test(day))throw Object.assign(new Error('تاريخ المعاينة غير صحيح.'),{status:400});
 const mode=employee.data_environment==='production'?'production':'training',policy=await effectiveEmployeeAttendancePolicy(env,employee,mode,day);
 const activeRules=(policy.rules||[]).map(r=>({id:r.id,rule_type:r.rule_type,label:r.label,start_date:r.start_date,end_date:r.end_date,start_time:r.start_time,end_time:r.end_time,policy_payload:r.policy_payload||{},notes:r.notes||null}));
 const branch=(await rest(env,'branches?id=eq.'+enc(employee.branch_id)+'&select=id,name&limit=1').catch(()=>[]))?.[0]||null;
 return {ok:true,work_date:day,employee:{id:employee.id,name:employee.name,employee_code:employee.employee_code,branch_id:employee.branch_id},branch,attendance_mode:policy.attendance_mode,mobile_enabled:policy.mobile_enabled,exceptions:policy.exceptions||{},active_rules:activeRules,policy:{attendance_mode:policy.attendance_mode,mobile_geofence_enabled:policy.mobile_geofence_enabled!==false,mobile_geofence_radius_m:Number(policy.mobile_geofence_radius_m||100),mobile_max_accuracy_m:Number(policy.mobile_max_accuracy_m||120),mobile_require_trusted_device:policy.mobile_require_trusted_device!==false,mobile_require_selfie:policy.mobile_require_selfie===true,mobile_require_dynamic_qr:policy.mobile_require_dynamic_qr===true,policy_version_id:policy.policy_version_id||null,policy_effective_from:policy.policy_effective_from||null,policy_effective_to:policy.policy_effective_to||null,notes:policy.notes||null}};
}
