...
async function createSelfServiceRequest(env,{account,employee=null,deviceId=null,pin=null,type,reason='',evidence={},mode='training'}){
 let path='attendance_self_service_biometric_requests?staff_user_id=eq.'+enc(account.id)+'&request_type=eq.'+enc(type)+'&status=eq.pending&select=*&order=requested_at.desc&limit=20';
