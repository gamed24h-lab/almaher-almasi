...
const TYPO_KEYS=new Set(['companyAr','companyEn','serviceLine','nameAr','nameEn','jobTitleAr','jobTitleEn','numberLabel','cardNumber','meta','slogan','footerAr','footerEn','approvalAr','approvalEn']);
const TYPO_FIELDS=new Set(['fontFamily','fontSizePt','fontWeight','color','align','letterSpacingPt','lineHeight']);
const FONT_FAMILIES=new Set(['Arial','Tahoma','Cairo','Noto Sans Arabic','Noto Kufi Arabic','Segoe UI']);
const TEXT_ALIGN=new Set(['left','center','right','start','end']);
...
function cleanTemplateConfig(value){
 ...
 if(d?.typography!==undefined){if(!plain(d.typography))throw Object.assign(new Error('إعدادات الخطوط غير صالحة.'),{status:400});for(const [ori,elements] of Object.entries(d.typography)){if(!ORIENTATIONS.has(ori)||!plain(elements))throw Object.assign(new Error('اتجاه تنسيق الخطوط غير صالح.'),{status:400});for(const [element,props] of Object.entries(elements)){if(!TYPO_KEYS.has(element)||!plain(props))throw Object.assign(new Error(`عنصر النص ${element} غير مدعوم.`),{status:400});for(const [field,val] of Object.entries(props)){if(!TYPO_FIELDS.has(field))throw Object.assign(new Error(`خاصية الخط ${field} غير مدعومة.`),{status:400});if(field==='fontFamily'){if(typeof val!=='string'||!FONT_FAMILIES.has(val))throw Object.assign(new Error('نوع الخط غير مسموح.'),{status:400});continue}if(field==='color'){if(typeof val!=='string'||!/^#[0-9a-f]{3,8}$/i.test(val))throw Object.assign(new Error('لون النص غير صالح.'),{status:400});continue}if(field==='align'){if(!TEXT_ALIGN.has(val))throw Object.assign(new Error('محاذاة النص غير صالحة.'),{status:400});continue}const n=Number(val);if(!Number.isFinite(n))throw Object.assign(new Error(`قيمة ${field} للنص ${element} غير صالحة.`),{status:400});if(field==='fontSizePt'&&(n<4||n>40))throw Object.assign(new Error('حجم الخط خارج النطاق.'),{status:400});if(field==='fontWeight'&&(n<300||n>900||n%100!==0))throw Object.assign(new Error('وزن الخط غير صالح.'),{status:400});if(field==='letterSpacingPt'&&(n<-2||n>8))throw Object.assign(new Error('تباعد الحروف خارج النطاق.'),{status:400});if(field==='lineHeight'&&(n<.8||n>3))throw Object.assign(new Error('ارتفاع السطر خارج النطاق.'),{status:400})}}}}
 return out;
}
