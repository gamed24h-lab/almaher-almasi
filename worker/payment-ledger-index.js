...
const txKind=x=>String(x?.transaction_type??x?.type??x?.kind??x?.category??x?.action??'').trim().toLowerCase();
...
async function insertTransaction(env,row){const b=base(env),h=headers(env);const r=await fetch(`${b}/rest/v1/transactions`,{method:'POST',headers:{...h,Prefer:'return=representation'},body:JSON.stringify(row)});const out=await parse(r);if(r.ok){const saved=Array.isArray(out)?out[0]||row:out;return {ok:true,row:saved}}return {ok:false,error:out?.message||out?.details||'تعذر إنشاء حركة التحصيل'}}
...
async function receiptData(request,env,ref,mode='view'){
  ...
  const tx=await one(env,'transactions',`reference_no=eq.${enc(ref)}&select=*&limit=1`);
  ...
  const booking=await one(env,'bookings',`id=eq.${enc(tx.booking_id)}&select=id,booking_number,branch_id,customer_name,customer_phone,total_price,paid_amount,payment_method,financial_status,status&limit=1`);
  ...
  return {data:{receipt_no:ref,transaction:tx,booking,branch,collector:{id:ev?.actor_id||null,name:ev?.actor_name||null,role:ev?.actor_role||null},payment_method:ev?.metadata?.payment_method||tx?.payment_method||booking?.payment_method||null,payment_reference:ev?.metadata?.payment_reference||null,previous_paid:Number(ev?.metadata?.previous_paid||0),new_paid:Number(ev?.metadata?.new_paid||booking?.paid_amount||0),created_at:ev?.created_at||tx.created_at}};
}
      ...
      if(no){
        ...
        if(before){
          ...
          if(delta>0.001){
            ...
            const pending={booking_id:before.id,branch_id:before.branch_id,transaction_type:'payment',amount:delta,payment_method:input.paymentMethod||input.payment_method||before.payment_method||null,status:'pending',reference_no:ref,created_at:ts};
            ...
            await audit(env,actor,before,{receipt_no:ref,amount:delta,previous_paid:oldPaid,new_paid:nextPaid,payment_method:input.paymentMethod||input.payment_method||before.payment_method||null,payment_reference:input.paymentReference||input.payment_reference||null,transaction_id:created.row?.id||null,ledger_status:posted?'posted':'pending'});
