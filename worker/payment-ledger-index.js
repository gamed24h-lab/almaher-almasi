...
const canFinance=a=>!!a&&(elevated(a)||a.permissions?.finance||a.permissions?.payments||a.permissions?.viewBookings||a.permissions?.editBookings);
...
async function receiptData(request,env,ref){
  const actor=await actorFrom(request,env);if(!actor)return {error:json({error:'انتهت الجلسة.'},401)};if(!canFinance(actor))return {error:json({error:'لا توجد صلاحية عرض سندات القبض.'},403)};
...
function receiptHtml(d){const b=d.booking||{},br=d.branch||{},tx=d.transaction||{};return `<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>سند قبض ${esc(d.receipt_no)}</title><style>@page{size:A4 portrait;margin:12mm}body{font-family:Arial,Tahoma,sans-serif;color:#14213d;margin:0}.sheet{max-width:760px;margin:auto;border:1px solid #d9e1ea;border-radius:16px;padding:24px}.head{text-align:center;border-bottom:2px solid #14213d;padding-bottom:14px;margin-bottom:18px}.head h1{margin:0 0 6px;font-size:26px}.head p{margin:3px 0;color:#5b6472}.grid{display:grid;grid-template-columns:1fr 1fr;gap:10px 18px}.item{border-bottom:1px solid #e6ebf0;padding:8px 0}.item span{display:block;color:#667085;font-size:12px}.item b{display:block;margin-top:4px}.amount{margin:20px 0;text-align:center;border:2px solid #14213d;border-radius:14px;padding:16px}.amount span{display:block;color:#667085}.amount strong{font-size:28px}.foot{margin-top:24px;display:grid;grid-template-columns:1fr 1fr;gap:30px}.sig{border-top:1px solid #667085;padding-top:8px;text-align:center;color:#667085}@media print{button{display:none}}</style></head><body><div class="sheet"><div class="head"><h1>سند قبض</h1><p>شركة الماهر الماسي</p><p>${esc(br.name||'')} ${br.address?`— ${esc(br.address)}`:''}</p></div><div class="grid"><div class="item"><span>رقم السند</span><b>${esc(d.receipt_no)}</b></div><div class="item"><span>التاريخ</span><b>${esc(new Date(d.created_at||Date.now()).toLocaleString('ar-SA'))}</b></div><div class="item"><span>رقم الحجز</span><b>${esc(b.booking_number||'—')}</b></div><div class="item"><span>العميل</span><b>${esc(b.customer_name||'—')}</b></div><div class="item"><span>الجوال</span><b>${esc(b.customer_phone||'—')}</b></div><div class="item"><span>طريقة الدفع</span><b>${esc(d.payment_method||'—')}</b></div><div class="item"><span>مرجع العملية</span><b>${esc(d.payment_reference||'—')}</b></div><div class="item"><span>الموظف</span><b>${esc(d.collector?.name||'—')}</b></div></div><div class="amount"><span>المبلغ المستلم</span><strong>${Number(tx.amount||0).toFixed(2)} ريال</strong></div><div class="grid"><div class="item"><span>المدفوع قبل العملية</span><b>${Number(d.previous_paid||0).toFixed(2)} ريال</b></div><div class="item"><span>المدفوع بعد العملية</span><b>${Number(d.new_paid||0).toFixed(2)} ريال</b></div><div class="item"><span>إجمالي الحجز</span><b>${Number(b.total_price||0).toFixed(2)} ريال</b></div><div class="item"><span>حالة الحركة</span><b>${esc(tx.status||'posted')}</b></div></div><div class="foot"><div class="sig">توقيع الموظف</div><div class="sig">توقيع العميل</div></div></div><script>window.addEventListener('load',()=>setTimeout(()=>window.print(),250))</script></body></html>`}

export default {async fetch(request,env,ctx){
  const u=new URL(request.url);
  if(u.pathname==='/api/payments/receipt'&&request.method==='GET'){
    const ref=String(u.searchParams.get('receipt_no')||'').trim();if(!ref)return json({error:'رقم سند القبض مطلوب.'},400);const r=await receiptData(request,env,ref);if(r.error)return r.error;return json({ok:true,...r.data});
  }
  if(u.pathname==='/api/payments/receipt/print'&&request.method==='GET'){
    const ref=String(u.searchParams.get('receipt_no')||'').trim();if(!ref)return json({error:'رقم سند القبض مطلوب.'},400);const r=await receiptData(request,env,ref);if(r.error)return r.error;return new Response(receiptHtml(r.data),{status:200,headers:{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-store','Content-Security-Policy':"default-src 'none'; style-src 'unsafe-inline'; script-src 'unsafe-inline'"}});
  }
      ...
      if(no){
        ...
        if(before){
          ...
          if(delta>0.001){
            ...
            if(out&&typeof out==='object')return json({...out,payment_receipt:{receipt_no:ref,amount:delta,previous_paid:oldPaid,new_paid:nextPaid,transaction_id:created.row?.id||null,status:posted?'posted':'pending',print_url:`/api/payments/receipt/print?receipt_no=${encodeURIComponent(ref)}`}},downstream.status);
            return downstream;
          }
        }
      }
