      ...
      if(no){
        ...
        if(before){
          ...
          if(delta>0.001){
            ...
            if(out&&typeof out==='object')return json({...out,payment_receipt:{receipt_no:ref,amount:delta,previous_paid:oldPaid,new_paid:nextPaid,transaction_id:created.row?.id||null,status:posted?'posted':'pending'}},downstream.status);
            return downstream;
          }
        }
      }
