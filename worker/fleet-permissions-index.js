...
function requiredPermission(table,action){
 if(table==='drivers'){if(action==='insert')return ['addDrivers','manageDrivers'];if(action==='update')return ['editDrivers','manageDrivers'];if(action==='delete')return ['deleteDrivers','manageDrivers']}
 if(table==='vehicles'){if(action==='insert')return ['addVehicles','vehicles'];if(action==='update')return ['editVehicles','vehicles'];if(action==='delete')return ['deleteVehicles','vehicles']}
...
async function directDriverList(env,actor){
 ...
 const params=new URLSearchParams();params.set('select','id,name,phone,national_id,license_no,license_expiry,status,notes,branch_id,created_at');params.set('order','name.asc');if(!allBranches(actor))params.set('branch_id',`eq.${actor.branch_id}`);
 const r=await fetch(`${b}/rest/v1/drivers?${params.toString()}`,{headers:headers(env)}),out=await parse(r);if(!r.ok)return json({error:out?.message||'تعذر قراءة قائمة السائقين.',code:'DRIVERS_READ_FAILED'},r.status>=400&&r.status<600?r.status:502);return json({ok:true,drivers:Array.isArray(out)?out:[],count:Array.isArray(out)?out.length:0,scope:allBranches(actor)?'all':'branch'});
}
async function directDriverWrite(request,env,actor,body){
 const b=base(env);if(!b||!env.SUPABASE_SERVICE_ROLE_KEY)return json({error:'إعدادات Supabase على الخادم غير مكتملة.'},500);const action=String(body?.action||''),id=String(body?.id||''),h=headers(env);let row=body?.row&&typeof body.row==='object'?{...body.row}:{};
 if(!allBranches(actor)){if(!actor.branch_id)return json({error:'حساب الموظف غير مرتبط بفرع.'},409);if(action==='insert')row.branch_id=actor.branch_id;if(action==='update'||action==='delete'){if(!id)return json({error:'معرّف السائق مطلوب.'},400);const rr=await fetch(`${b}/rest/v1/drivers?id=eq.${encodeURIComponent(id)}&select=id,branch_id&limit=1`,{headers:h}),rb=await parse(rr);if(!rr.ok)return json({error:rb?.message||'تعذر التحقق من السائق.'},502);const current=Array.isArray(rb)?rb[0]:null;if(!current)return json({error:'السائق غير موجود.'},404);if(String(current.branch_id||'')!==String(actor.branch_id||''))return json({error:'لا يمكن تعديل أو حذف سائق تابع لفرع آخر.'},403);if(action==='update')row.branch_id=actor.branch_id}}
 if(action==='insert'){const r=await fetch(`${b}/rest/v1/drivers`,{method:'POST',headers:{...h,Prefer:'return=representation'},body:JSON.stringify(row)}),out=await parse(r);if(!r.ok)return json({error:out?.message||'تعذر إضافة السائق.'},r.status>=400&&r.status<600?r.status:502);const created=Array.isArray(out)?out[0]:out;if(!created?.id)return json({error:'تم إرسال طلب إضافة السائق لكن الخادم لم يرجع سجلًا يمكن التحقق منه.',code:'DRIVER_WRITE_UNVERIFIED'},502);return json({ok:true,row:created,verified:true})}
 if(action==='update'){if(!id)return json({error:'معرّف السائق مطلوب.'},400);const r=await fetch(`${b}/rest/v1/drivers?id=eq.${encodeURIComponent(id)}`,{method:'PATCH',headers:{...h,Prefer:'return=representation'},body:JSON.stringify(row)}),out=await parse(r);if(!r.ok)return json({error:out?.message||'تعذر تعديل السائق.'},r.status>=400&&r.status<600?r.status:502);const updated=Array.isArray(out)?out[0]:out;if(!updated?.id)return json({error:'تم إرسال تعديل السائق لكن الخادم لم يرجع السجل بعد التعديل.',code:'DRIVER_UPDATE_UNVERIFIED'},502);return json({ok:true,row:updated,verified:true})}
...
async function directTripVehicleWrite(env,body){
 ...
 if(action==='delete'){if(!id)return json({error:'معرّف ربط الباص مطلوب.'},400);const r=await fetch(`${b}/rest/v1/trip_vehicles?id=eq.${encodeURIComponent(id)}`,{method:'DELETE',headers:{...h,Prefer:'return=representation'}}),out=await parse(r);if(!r.ok)return json({error:out?.message||'تعذر فك ربط الباص.'},r.status);return json({ok:true,row:Array.isArray(out)?out[0]:out})}
 return json({error:'عملية ربط الباص غير مدعومة.'},400);
}
