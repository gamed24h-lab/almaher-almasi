...
async function handleIdStudio(env,me,body){const action=txt(body.action);requirePerm(me,'id_studio_access');
  ...
  if(action==='id_studio_printer_profile_save'){
    requirePerm(me,'id_card_manage_settings');const p=body.profile||{},id=txt(p.id),owner=actorId(me);if(!owner)throw Object.assign(new Error('تعذر تحديد المستخدم مالك Profile الطابعة.'),{status:400});if(id&&!UUID_RE.test(id))throw Object.assign(new Error('معرّف Profile الطابعة غير صالح.'),{status:400});
    const offsetX=p.offset_x_mm===undefined||p.offset_x_mm===null||p.offset_x_mm===''?0:Number(p.offset_x_mm),offsetY=p.offset_y_mm===undefined||p.offset_y_mm===null||p.offset_y_mm===''?0:Number(p.offset_y_mm),scaleX=p.scale_x===undefined||p.scale_x===null||p.scale_x===''?1:Number(p.scale_x),scaleY=p.scale_y===undefined||p.scale_y===null||p.scale_y===''?1:Number(p.scale_y);
    if(!Number.isFinite(offsetX)||!Number.isFinite(offsetY)||offsetX < -20||offsetX > 20||offsetY < -20||offsetY > 20)throw Object.assign(new Error('إزاحة الطابعة يجب أن تكون بين -20 و20 مم.'),{status:400});
    if(!Number.isFinite(scaleX)||!Number.isFinite(scaleY)||scaleX < .9||scaleX > 1.1||scaleY < .9||scaleY > 1.1)throw Object.assign(new Error('مقياس الطباعة يجب أن يكون بين 0.9 و1.1.'),{status:400});
    const row={name:txt(p.name)||'الطابعة الافتراضية',printer_name:txt(p.printer_name)||null,offset_x_mm:offsetX,offset_y_mm:offsetY,scale_x:scaleX,scale_y:scaleY,is_default:!!p.is_default,owner_user_id:owner,updated_at:new Date().toISOString()};let profile;
    if(id){const out=await rest(env,`id_card_printer_profiles?id=eq.${enc(id)}&owner_user_id=eq.${enc(owner)}`,{method:'PATCH',body:row,prefer:'return=representation'});profile=out?.[0];if(!profile)throw Object.assign(new Error('Profile الطابعة غير موجود أو لا يخص المستخدم الحالي.'),{status:404})}else{profile=(await rest(env,'id_card_printer_profiles',{method:'POST',body:row,prefer:'return=representation'}))?.[0]}
    if(row.is_default&&profile?.id)await rest(env,`id_card_printer_profiles?owner_user_id=eq.${enc(owner)}&id=neq.${enc(profile.id)}&is_default=eq.true`,{method:'PATCH',body:{is_default:false,updated_at:new Date().toISOString()},prefer:'return=minimal'});
