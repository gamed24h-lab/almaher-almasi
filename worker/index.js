  ...
  exports.handler=async(event)=>{
    ...
    try{
      ...
      if(action==='regression_run'){
        if(!isManager(actor))return{statusCode:403,headers:H,body:JSON.stringify({error:'Manager/developer only'})};const required=['branches','trips','bookings','booking_passengers','staff_users','trip_branches','scan_events','room_assignments','seat_assignments','notifications','export_jobs','system_releases','post_trip_ratings','presence_sessions'];const checks=[];for(const t of required){const r=await fetch(`${url}/rest/v1/${t}?select=*&limit=1`,{headers:authHeaders(key)});checks.push({name:`table:${t}`,ok:r.ok,status:r.status})}const h=await health(url,key);checks.push({name:'health',ok:h.ok,status:h.latency_ms});const ok=checks.every(x=>x.ok);const saved=await upsert(url,key,'regression_runs',{release_version:'1.2.0',status:ok?'pass':'fail',checks,run_by:actor.name||'',created_at:now()});return{statusCode:200,headers:H,body:JSON.stringify({ok,checks,row:saved?.[0]||saved})};
      }
...
async function nextApi(request,env){
  ...
  if(path==='/api/health') return nextJson({ok:true,service:'almaher-next',architecture:'react-vite-worker',version:'1.2.0'});
