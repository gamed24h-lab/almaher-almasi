...
function __load_create_booking(){
  ...
  exports.handler = async (event) => {
    ...
    const femaleValue=v=>['female','f','أنثى'].includes(String(v||'').trim().toLowerCase());
    if(String(b.accommodation_type||'').toLowerCase()==='shared'&&(femaleValue(b.customer_gender)||passengers.some(x=>femaleValue(x?.gender))))return{statusCode:400,headers,body:JSON.stringify({error:'السكن المشترك غير متاح للنساء. اختر غرفة خاصة أو بدون سكن.'})};
...
function __load_staff_admin(){
  ...
  exports.handler=async(event)=>{
   ...
   try{
    ...
    if(action==='company_settings_get'){
     if(session.role!=='مدير عام'&&session.role!=='developer'&&!has(session,'manageBranches'))return{statusCode:403,headers:H,body:JSON.stringify({error:'إدارة بيانات الشركة للإدارة العليا فقط'})};
     const rr=await fetch(`${url}/rest/v1/system_settings?key=eq.company.profile.v1&select=value&limit=1`,{headers:sh});const rows=await rr.json().catch(()=>[]);if(!rr.ok)return{statusCode:500,headers:H,body:JSON.stringify({error:rows?.message||'تعذر تحميل بيانات الشركة'})};return{statusCode:200,headers:H,body:JSON.stringify({ok:true,settings:Array.isArray(rows)&&rows[0]?.value?rows[0].value:{}})};
    }
    if(action==='company_settings_save'){
     if(session.role!=='مدير عام'&&session.role!=='developer'&&!has(session,'manageBranches'))return{statusCode:403,headers:H,body:JSON.stringify({error:'إدارة بيانات الشركة للإدارة العليا فقط'})};
     const x=p.settings&&typeof p.settings==='object'?p.settings:{};const settings={company_name_ar:String(x.company_name_ar||'شركة الماهر الماسي').slice(0,160),company_name_en:String(x.company_name_en||'Al-Maher Al-Masi Company').slice(0,160),commercial_registration:String(x.commercial_registration||'').slice(0,80),tax_number:String(x.tax_number||'').slice(0,80),email:String(x.email||'').slice(0,160),phone:String(x.phone||'').slice(0,80),website:String(x.website||'').slice(0,240),slogan:String(x.slogan||'نُيسر دربك... لنطمئن قلبك').slice(0,240),address:String(x.address||'').slice(0,500),updated_by_name:String(session.name||session.username||'').slice(0,160),updated_at:new Date().toISOString()};
     const rr=await fetch(`${url}/rest/v1/system_settings?on_conflict=key`,{method:'POST',headers:{...sh,Prefer:'resolution=merge-duplicates,return=minimal'},body:JSON.stringify([{key:'company.profile.v1',value:settings,updated_by:isUuid(session.id)?session.id:null,updated_at:new Date().toISOString()}])});if(!rr.ok){const e=await rr.json().catch(()=>({}));return{statusCode:500,headers:H,body:JSON.stringify({error:e?.message||'تعذر حفظ بيانات الشركة'})}}return{statusCode:200,headers:H,body:JSON.stringify({ok:true,settings})};
    }
...
function __load_v9_admin_data(){
  ...
  const READ_MAP={
    ...
    suppliers:['suppliers','supplier_contracts','supplier_payables','agents','agent_allocations'],
    ...
    try{
      ...
      if(event.httpMethod==='POST'){
        ...
        if(p.action==='scan'){
          ...
          if(['cancelled','refunded'].includes(String(booking.status||'').toLowerCase()))return{statusCode:409,headers:H,body:JSON.stringify({error:'التذكرة/الحجز ملغى وغير صالح للمسح',result:'cancelled'})};
        ...
        if(p.action==='create_operational_snapshot'){
          ...
          let rr=await fetch(`${url}/rest/v1/backup_runs`,{method:'POST',headers:{...sh,Prefer:'return=representation'},body:JSON.stringify([rec])});let rb=await rr.json().catch(()=>[]);if(!rr.ok&&/details.*column|Could not find.*details/i.test(String(rb?.message||''))){const legacy={...rec};delete legacy.details;rr=await fetch(`${url}/rest/v1/backup_runs`,{method:'POST',headers:{...sh,Prefer:'return=representation'},body:JSON.stringify([legacy])});rb=await rr.json().catch(()=>[]);}if(!rr.ok)throw new Error(rb?.message||'تعذر تسجيل اللقطة');const signed_url=await v970Signed('almaher-backups',path,900);return{statusCode:200,headers:H,body:JSON.stringify({ok:true,row:Array.isArray(rb)?rb[0]:rb,signed_url})};
        }
...
function __load_mega_completion(){
  ...
  exports.handler=async(event)=>{
    ...
    if(action==='ticket_terms'){const saved=await setting(url,key,'ticket.terms.v1');const terms=Array.isArray(saved?.terms)?saved.terms:['الحضور قبل موعد الانطلاق بوقت كافٍ.','الاحتفاظ بالتذكرة والهوية/الإقامة أثناء الرحلة.','التذكرة الملغاة أو المستبدلة غير صالحة للاستخدام.'];return{statusCode:200,headers:H,body:JSON.stringify({ok:true,terms})};}
    if(action==='save_ticket_terms'){if(!(actor.role==='developer'||actor.role==='مدير عام'||actor.permissions?.all))return{statusCode:403,headers:H,body:JSON.stringify({error:'إدارة شروط التذكرة للإدارة العليا فقط'})};const terms=(Array.isArray(p.terms)?p.terms:[]).map(x=>String(x||'').trim()).filter(Boolean).slice(0,30);await saveSetting(url,key,'ticket.terms.v1',{terms,updated_by_name:actor.name,updated_at:now()},actor);return{statusCode:200,headers:H,body:JSON.stringify({ok:true,terms})};}
    try{
      ...
      if(action==='status'){const emergency={maintenance:await setting(url,key,'emergency.maintenance'),read_only:await setting(url,key,'emergency.read_only'),stop_new_bookings:await setting(url,key,'emergency.stop_new_bookings')};return{statusCode:200,headers:H,body:JSON.stringify({ok:true,build:'1.2.0',providers:await providerStatus(env),emergency,actor:{name:actor.name,role:actor.role,branch_id:uuidOrNull(actor.branch_id)}})}}
...
async function nextApi(request,env){
  ...
  if(path==='/api/health') return nextJson({ok:true,service:'almaher-next',architecture:'react-vite-worker',version:'1.3.0'});
