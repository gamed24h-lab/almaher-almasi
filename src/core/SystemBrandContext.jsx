...
export function SystemBrandProvider({children}){
 ...
 useEffect(()=>{api.mega('public_brand_profile',{},'GET').then(x=>setValue({profile:{...DEFAULT.profile,...(x?.profile||{})},labels:{...DEFAULT.labels,...(x?.labels||{})},config:{...DEFAULT.config,...(x?.config||{})}})).catch(()=>{})},[]);
 const v=useMemo(()=>value,[value]);return <C.Provider value={v}>{children}</C.Provider>
}
export function useSystemBrand(){return useContext(C)||DEFAULT}
