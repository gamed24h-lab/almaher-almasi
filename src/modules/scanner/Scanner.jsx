...
export default function Scanner({tripId}){
 const [code,setCode]=useState(''),[mode,setMode]=useState(''),[result,setResult]=useState(null),[error,setError]=useState(''),[camera,setCamera]=useState(false);
 const video=useRef(null),stream=useRef(null),qrRef=useRef(null),running=useRef(false);
 useEffect(()=>()=>stopCamera(),[]);
 async function scan(v=code){if(!mode){setError('اختر وضع المسح أولًا.');return}if(!String(v).trim())return;setError('');try{const x=await api.moduleWrite({action:'scan',code:String(v).trim(),scan_mode:mode,trip_id:tripId||undefined,device_id:navigator.userAgent.slice(0,120)});setResult(x);tone(x.result==='duplicate'?440:880)}catch(e){setError(e.message);tone(220)}}
