...
import {bookingFinanceNumbers,bookingFinancialState,paymentMethodLabel} from '../../lib/bookingFinance.js';
...
const statusAr=v=>({new:'جديد',confirmed:'مؤكد',paid:'مسدد',pending:'قيد المراجعة',cancelled:'ملغي',refunded:'مسترد'})[lower(v)]||str(v||'مؤكد');
...
export default function TicketPage({bookingNo,go}){
 ...
 const passengers=data.passengers.filter(x=>str(x.booking_id)===str(b?.id)&&lower(x.status)!=='cancelled');
 ...
 const [refundSummary,setRefundSummary]=useState({refunded:0,loaded:false});
 ...
 useEffect(()=>{if(!b?.booking_number){setRefundSummary({refunded:0,loaded:true});return}let alive=true;setRefundSummary(x=>({...x,loaded:false}));api.admin({action:'refund_quote',booking_number:b.booking_number}).then(x=>{if(alive)setRefundSummary({refunded:Number(x?.refunded_amount||0),loaded:true})}).catch(()=>{if(alive)setRefundSummary({refunded:0,loaded:true})});return()=>{alive=false}},[b?.booking_number]);
 ...
 const seatByPassenger=useMemo(()=>{const map=new Map();for(const x of ops?.seat_assignments||[]){if(!['assigned','hold'].includes(lower(x.status||'assigned')))continue;if(str(x.booking_id)===str(b?.id)||passengers.some(p=>str(p.id)===str(x.passenger_id))){const k=str(x.passenger_id||'booking');const arr=map.get(k)||[];arr.push(x);map.set(k,arr)}}return map},[ops,b?.id,passengers]);
 const roomByPassenger=useMemo(()=>{const currentPassengerIds=new Set(passengers.map(p=>str(p.id)));const roomMap=new Map((ops?.hotel_rooms||[]).map(x=>[str(x.id),x]));const tripHotelMap=new Map((ops?.trip_hotels||[]).map(x=>[str(x.id),x]));const hotelMap=new Map((ops?.hotels||[]).map(x=>[str(x.id),x]));const out=new Map();for(const a of ops?.room_assignments||[]){if(!currentPassengerIds.has(str(a.passenger_id))||lower(a.status||'assigned')==='released')continue;const room=roomMap.get(str(a.hotel_room_id));if(!room)continue;const th=tripHotelMap.get(str(room.trip_hotel_id));const hotel=hotelMap.get(str(th?.hotel_id));out.set(str(a.passenger_id),{assignment:a,room,tripHotel:th,hotel})}return out},[ops,passengers]);
 if(!b)return <Card>الحجز غير موجود في البيانات الحالية.</Card>;
 const refunded=Number(refundSummary.refunded||0),finance=bookingFinanceNumbers(b,refunded),financialState=bookingFinancialState(b,refunded);
 const {gross:historicalCollected,paid,total,remaining,credit}=finance;
 ...
 const housing=housingEntries[0];const hasActualHousing=housingEntries.length>0;
 ...
 const returnOnlyHousingDays=mode==='returnonly'&&hasActualHousing?(housingType==='private'&&requestedPrivateRooms.length?Math.max(...requestedPrivateRooms.map(x=>x.days),1):housingDays):0;
 const calculatedReturnOnlyCheckIn=returnOnlyHousingDays>0?minusDays(returnDate,returnOnlyHousingDays):'';
 const hotelName=hasActualHousing?hotelNames.join('، '):'';
 ...
 const checkIn=hasActualHousing?shortDate(mode==='returnonly'?first(housing?.tripHotel?.check_in_date,housing?.tripHotel?.checkin_date,snap?.housingCheckInDate,snap?.checkIn,calculatedReturnOnlyCheckIn):first(housing?.tripHotel?.check_in_date,housing?.tripHotel?.checkin_date,snap?.checkIn)):'';
 const checkOut=hasActualHousing?shortDate(mode==='returnonly'?first(housing?.tripHotel?.check_out_date,housing?.tripHotel?.checkout_date,snap?.housingCheckOutDate,snap?.checkOut,returnDate):first(housing?.tripHotel?.check_out_date,housing?.tripHotel?.checkout_date,snap?.checkOut,returnDate)):'';
 ...
 const paymentLabel=financialState.label;const paymentMethod=paymentMethodLabel(b.payment_method,printLanguage);
 ...
 const wa=()=>{const msg=[`الماهر الماسي للسفر والسياحة`,`تذكرة سفر: ${b.booking_number}`,`العميل: ${b.customer_name||''}`,`نوع الرحلة: ${journeyLabel(b.journey_mode)||'—'}`,showOutbound&&outboundRouteLabel?`الذهاب: ${outboundRouteLabel}`:'',showOutbound&&trip?.departure_date?`تاريخ الذهاب: ${weekdayAr(trip.departure_date)} ${trip.departure_date} ${trip.departure_time||''}`:'',showReturn&&returnRouteLabel?`العودة: ${returnRouteLabel}`:'',showReturn&&returnDate?`تاريخ العودة: ${weekdayAr(returnDate)} ${returnDate} ${returnTime||''}`:'',`السكن: ${housingTypeLabel}`,housingType==='private'&&privateRoomPlanLabel?`تفاصيل الغرف: ${privateRoomPlanLabel}`:'',`قيمة الحجز: ${money(total)}`,`إجمالي التحصيل التاريخي: ${money(historicalCollected)}`,refunded>0?`تم استرداده: ${money(refunded)}`:'',`المدفوع الصافي: ${money(paid)}`,credit>0?`رصيد مستحق للعميل: ${money(credit)}`:remaining?`المتبقي: ${money(remaining)}`:`الحالة المالية: ${paymentLabel}`].filter(Boolean).join('\n');window.open(`https://wa.me/${phoneWa(b.customer_phone)}?text=${encodeURIComponent(msg)}`,'_blank')};
 return <><PageHeader title="تذكرة سفر" subtitle={`${L('bookingNo')} ${b.booking_number}`} actions={<><Button onClick={()=>go('/bookings/'+b.booking_number)}><ArrowRight size={16}/> {L('back')}</Button><label className="btn" style={{gap:7}}><Languages size={16}/><select value={printLanguage} onChange={e=>{setLanguageTouched(true);setPrintLanguage(e.target.value)}} style={{border:0,padding:0,width:'auto',background:'transparent'}}>{PRINT_LANGUAGES.map(([code,label])=><option key={code} value={code}>{label}</option>)}</select></label><Button onClick={()=>doPrint('a4')}><Printer size={16}/> A4</Button><Button onClick={()=>doPrint('80')}><Printer size={16}/> 80mm</Button><Button onClick={()=>doPrint('58')}><Printer size={16}/> 58mm</Button><Button variant="primary" onClick={wa}><MessageCircle size={16}/> إرسال التذكرة واتساب</Button></>}/><ErrorBox error={error}/>{!ops&&!error&&<Loading text="استكمال بيانات المقعد والسكن..."/>}<article className={`ticket-page ${dense} ${ticketCancelled?'ticket-cancelled':''}`} dir={dir} lang={printLanguage}>{ticketCancelled&&<div className="ticket-cancelled-banner">هذه التذكرة ملغاة</div>}<header className="ticket-head"><div className="ticket-brand-lockup"><img className="ticket-logo" src={branchLogo(branch)} alt={`شعار ${branch?.name||'الماهر الماسي'}`}/><span className="ticket-brand-english">ALMAHER ALMASI · TRAVEL & TOURISM</span></div><div className="ticket-meta-card"><span className="ticket-meta-label">رقم الحجز</span><b>{b.booking_number}</b><BookingBarcode value={b.booking_number}/><div className="ticket-meta-foot"><Badge tone={b.status==='cancelled'?'red':b.status==='new'?'orange':'green'}>{statusAr(b.status)}</Badge>{issuedDate&&<small>{issuedDate}</small>}</div></div></header><section className="ticket-title-band"><div className="ticket-title-rule"/><div><h2>تذكرة سفر</h2><span>TRAVEL TICKET</span></div><div className="ticket-title-rule"/></section><section className="ticket-customer-strip"><div><span>اسم العميل</span><strong>{b.customer_name||'—'}</strong></div><div><span>رقم الجوال</span><strong dir="ltr">{b.customer_phone||'—'}</strong></div><div><span>نوع الرحلة</span><strong>{journeyLabel(b.journey_mode)||routeLabel}</strong></div><div><span>حالة الدفع</span><strong>{paymentLabel}</strong><small>{paymentMethod}</small></div></section><section className="ticket-journeys">{showOutbound&&<JourneyBlock title={L('outbound')} trip={trip} date={trip?.departure_date} time={trip?.departure_time} boardingPoint={outboundBoardingPoint} boardingTime={outboundBoardingTime} unavailable={L('tripUnavailable')}/>} {showReturn&&<JourneyBlock title={L('return')} trip={returnInfo} date={returnDate} time={returnTime} boardingPoint={returnBoardingPoint} reverse={reverseReturn} unavailable={L('tripUnavailable')}/>}</section><section className="ticket-section ticket-passenger-section"><div className="ticket-section-heading"><h3>بيانات المسافرين</h3><span>{passengers.length} مسافر</span></div><div className="ticket-passenger-table-wrap"><table className="ticket-passenger-table"><thead><tr><th>#</th><th>الاسم</th><th>الجنس</th><th>الجنسية</th><th>رقم الهوية</th><th>المقعد</th><th>الغرفة</th></tr></thead><tbody>{passengers.map((p,i)=>{const seats=seatByPassenger.get(str(p.id))||[];const rawRoom=roomByPassenger.get(str(p.id));const room=housingType==='shared'&&lower(rawRoom?.room?.room_type)==='shared5'?rawRoom:housingType==='private'&&lower(rawRoom?.room?.room_type)==='private'?rawRoom:null;const seatsText=seats.length?seats.map(s=>s.seat_no||s.seat_number||'—').join('، '):'—';return <tr key={p.id||i}><td>{i+1}</td><td><b>{p.full_name||'—'}</b></td><td>{genderAr(first(p.gender,p.sex))}</td><td>{p.nationality||'—'}</td><td dir="ltr">{p.identity_number||p.passport_number||'—'}</td><td>{seatsText}</td><td dir="ltr">{room?displayRoom(room.room)||'—':'—'}</td></tr>})}</tbody></table></div></section><section className="ticket-section ticket-housing-section"><div className="ticket-section-heading"><h3>بيانات السكن</h3><span>{housingTypeLabel}</span></div><div className="ticket-housing-grid"><div><span>الفندق</span><strong>{hasHousing?(hotelName||'غير مسكن بعد'):'—'}</strong></div><div><span>نوع السكن</span><strong>{housingDetailLabel}{hasHousing&&roomNumberLabel?` · ${roomNumberLabel}`:''}</strong>{hasOperationalRoom&&<small>رقم الغرفة المعروض تشغيلي مؤقت، ويظهر رقم الغرفة الفعلي بعد تحديثه من الفندق.</small>}</div><div><span>الوصول</span><strong dir="ltr">{hasActualHousing?(checkIn||'—'):'—'}</strong></div><div><span>المغادرة</span><strong dir="ltr">{hasActualHousing?(checkOut||'—'):'—'}</strong></div></div></section><section className="ticket-section ticket-finance-section"><div className="ticket-section-heading"><h3>البيانات المالية</h3><span>{paymentLabel}</span></div><div className="ticket-finance-grid"><div><span>قيمة الحجز</span><b>{money(total)}</b></div><div><span>إجمالي التحصيل التاريخي</span><b>{money(historicalCollected)}</b></div>{refunded>0&&<div><span>تم استرداده</span><b>{money(refunded)}</b></div>}<div><span>المدفوع الصافي</span><b>{money(paid)}</b></div><div><span>{credit>0?'رصيد مستحق للعميل':'المتبقي'}</span><b>{money(credit>0?credit:remaining)}</b></div><div><span>عدد المسافرين</span><b>{passengers.length}</b></div></div></section><section className="ticket-bottom"><div className="ticket-qr-panel">{qr&&<div><img src={qr} alt="QR"/><span>رمز الحجز</span></div>}{locationQr&&<div><img src={locationQr} alt="QR location"/><span>موقع الصعود</span></div>}{portalQr&&<div><img src={portalQr} alt="QR customer portal"/><span>بوابة العميل</span></div>}</div><div className="ticket-terms"><h3>الشروط والأحكام</h3><ol>{(terms.length?terms:defaultTerms).map((x,i)=><li key={i}>{typeof x==='string'?x:x?.[printLanguage]||x?.text||x?.ar}</li>)}</ol></div></section><section className="ticket-branch-card"><div><MapPin size={14}/><span>{locationText||branch?.name||'الماهر الماسي'}</span></div>{phone&&<div><Phone size={14}/><span dir="ltr">{phone}</span></div>}<div><BusFront size={14}/><span>{routeLabel}</span></div></section><footer className="ticket-footer-strip">{showLegal&&license&&<div className="ticket-footer-license">رقم الترخيص: {license}</div>}<div className="ticket-blessing"><strong>على دروب الطاعة .. راحتكم غايتنا</strong><span>خطوة إلى الطاعات، ومغفرةٌ تمحو ما فات</span></div></footer>{config.show_profile_tickets!==false&&developer?.display_name&&<small className="ticket-developer-line">{L('developer')}: {developer.display_name}{developer.phone?` · ${developer.phone}`:''}</small>}</article></>;
}

function JourneyBlock({title,trip,date,time,boardingPoint,boardingTime,reverse=false,unavailable='بيانات الرحلة غير متاحة'}){if(!trip)return <div className="journey-block"><div className="journey-label"><BusFront size={18}/><strong>{title}</strong></div><span className="journey-unavailable">{unavailable}</span></div>;const from=reverse?(trip.to_city||trip.destination):(trip.from_city||trip.origin);const to=reverse?(trip.from_city||trip.origin):(trip.to_city||trip.destination);return <div className="journey-block"><div className="journey-label"><BusFront size={18}/><strong>{title}</strong><span>{trip.trip_code||''}</span></div><div className="journey-route"><b>{from||'—'}</b><span>←</span><b>{to||'—'}</b></div><div className="journey-details"><div><span>التاريخ</span><strong>{weekdayAr(date)} {date||'—'}</strong></div><div><span>الوقت</span><strong>{time||'—'}</strong></div>{boardingPoint&&<div><span>نقطة الصعود</span><strong>{boardingPoint}{boardingTime?` · ${boardingTime}`:''}</strong></div>}</div></div>}
