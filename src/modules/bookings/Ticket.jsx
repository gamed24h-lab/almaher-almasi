...
import {ArrowRight,Printer,MessageCircle,MapPin,BusFront,Hotel,Armchair,Phone,Languages} from 'lucide-react';
...
export default function TicketPage({bookingNo,go}){
 ...
 const [qr,setQr]=useState(''),[locationQr,setLocationQr]=useState(''),[ops,setOps]=useState(null),[terms,setTerms]=useState([]),[developer,setDeveloper]=useState({display_name:'Mohamed Abdelrahman Hassan',title:'مطور النظام',phone:'00966509444960'}),[error,setError]=useState(''),[printLanguage,setPrintLanguage]=useState('ar'),[languageTouched,setLanguageTouched]=useState(false);
 ...
 const roomByPassenger=useMemo(()=>{const roomMap=new Map((ops?.hotel_rooms||[]).map(x=>[str(x.id),x]));const tripHotelMap=new Map((ops?.trip_hotels||[]).map(x=>[str(x.id),x]));const hotelMap=new Map((ops?.hotels||[]).map(x=>[str(x.id),x]));const out=new Map();for(const a of ops?.room_assignments||[]){if(lower(a.status||'assigned')==='released')continue;const room=roomMap.get(str(a.hotel_room_id));if(!room)continue;const th=tripHotelMap.get(str(room.trip_hotel_id));const hotel=hotelMap.get(str(th?.hotel_id));out.set(str(a.passenger_id),{assignment:a,room,tripHotel:th,hotel})}return out},[ops]);
 ...
 const L=(key,f='')=>pt(printLanguage,key,f);const dir=printDirection(printLanguage);
 const doPrint=mode=>{document.body.dataset.printMode=mode;document.body.dataset.printLanguage=printLanguage;const cleanup=()=>{delete document.body.dataset.printMode;delete document.body.dataset.printLanguage;window.removeEventListener('afterprint',cleanup)};window.addEventListener('afterprint',cleanup);window.print();setTimeout(cleanup,1500)};
 ...
 const showOutbound=lower(b.journey_mode)!=='returnonly';const showReturn=['roundtrip','separate','returnonly'].includes(lower(b.journey_mode));const returnInfo=returnTrip||trip;const returnDate=returnTrip?.departure_date||trip?.return_date||trip?.departure_date;const returnTime=returnTrip?.departure_time||trip?.return_time||'';
 const defaultTerms=[L('defaultTerm1'),L('defaultTerm2'),L('defaultTerm3')];
 return <>
  <PageHeader title={L('ticket')} subtitle={`${L('bookingNo')} ${b.booking_number}`} actions={<>
   ...
   <Button onClick={()=>doPrint('a4')}><Printer size={16}/> A4</Button><Button onClick={()=>doPrint('80')}><Printer size={16}/> 80mm</Button><Button onClick={()=>doPrint('58')}><Printer size={16}/> 58mm</Button><Button variant="primary" onClick={wa}><MessageCircle size={16}/> واتساب</Button>
  </>}/>
  ...
  <article className={`ticket-page ${dense} ${ticketCancelled?'ticket-cancelled':''}`} dir={dir} lang={printLanguage}>
   <header className="ticket-head"><div className="ticket-brand-lockup"><img className="ticket-logo" src={branchLogo(branch)} alt={`شعار ${branch?.name||'الماهر الماسي'}`}/><div><h1>{branch?.name||'الماهر الماسي'}</h1><span>نُيسر دربك... لنطمئن قلبك</span></div></div><div className="ticket-number"><span>{L('bookingNo')}</span><b>{b.booking_number}</b><Badge tone={b.status==='cancelled'?'red':'green'}>{b.status||'confirmed'}</Badge></div></header>
   <section className="ticket-hero-grid"><div><span>{L('customer')}</span><strong>{b.customer_name}</strong><small>{b.customer_phone}</small></div><div><span>{L('journeyType')}</span><strong>{journeyLabel(b.journey_mode)}</strong><small>{branch?.name||''}</small></div><div><span>{L('housing')}</span><strong>{b.accommodation_label||b.accommodation_type||L('noHousing')}</strong><small>{b.accommodation_type==='private'?[privateTypeLabel&&`غرفة ${privateTypeLabel}`,`${Number(b.private_rooms||snap.privateRooms||1)} غرفة`,snap.housingDays&&`${snap.housingDays} ${L('day')}`].filter(Boolean).join(' · '):''}</small></div><div><span>{L('paymentStatus')}</span><strong>{remaining>0?`${L('remaining')} ${money(remaining)}`:L('paidFull')}</strong><small>{b.payment_method||''}</small></div></section>
   <section className="ticket-journeys">{showOutbound&&<JourneyBlock title={L('outbound')} trip={trip} date={trip?.departure_date} time={trip?.departure_time} boardingPoint={tripBranch?.boarding_point||b.snapshot?.boardingPoint} boardingTime={tripBranch?.boarding_time||b.snapshot?.boardingTime} unavailable={L('tripUnavailable')}/>} {showReturn&&<JourneyBlock title={L('return')} trip={returnInfo} date={returnDate} time={returnTime} reverse={!returnTrip&&lower(b.journey_mode)==='roundtrip'} unavailable={L('tripUnavailable')}/>}</section>
   <section className="ticket-passenger-section"><div className="ticket-section-title"><h3>{L('passengers')}</h3><span>{passengers.length} {L('passenger')}</span></div><div className="ticket-passengers">{passengers.map((p,i)=>{const seats=seatByPassenger.get(str(p.id))||[];const room=roomByPassenger.get(str(p.id));return <div className="ticket-passenger" key={p.id||i}><div className="ticket-pax-main"><b>{i+1}. {p.full_name}</b><span>{p.identity_number} · {p.nationality||''}</span></div><div className="ticket-pax-tags">{seats.length?seats.map((s,j)=><span key={s.id||j}><Armchair size={12}/> {s.seat_no||s.seat_number||'—'}{s.segment_type?` · ${s.segment_type}`:''}</span>):<span><Armchair size={12}/> {L('seatUnset')}</span>}{room&&<span><Hotel size={12}/> {room.hotel?.name||L('hotel')} · {L('room')} {room.room?.room_no||'—'}</span>}</div></div>})}</div></section>
   <section className="ticket-bottom"><div className="ticket-finance"><div><span>{L('total')}</span><b>{money(total)}</b></div><div><span>{L('paid')}</span><b>{money(paid)}</b></div><div><span>{L('remaining')}</span><b>{money(remaining)}</b></div></div><div className="ticket-qr-group">{qr&&<div><img src={qr} alt="QR"/><span>{L('bookingQr')}</span></div>}{locationQr&&<div><img src={locationQr} alt="QR location"/><span>{L('boardingLocation')}</span></div>}</div></section>
   <section className="ticket-terms"><h3>{L('terms')}</h3><ol>{(terms.length?terms:defaultTerms).map((x,i)=><li key={i}>{typeof x==='string'?x:x?.[printLanguage]||x?.text||x?.ar}</li>)}</ol></section>
   <footer className="ticket-contact"><div>{locationText&&<span><MapPin size={13}/> {locationText}</span>}{phone&&<span><Phone size={13}/> {phone}</span>}</div><small>{L('termsSnapshot')}</small><small>{L('developer')}: {developer.display_name||'Mohamed Abdelrahman Hassan'}{developer.phone?` · ${developer.phone}`:''}</small></footer>
  </article>
 </>;
}
function JourneyBlock({title,trip,date,time,boardingPoint,boardingTime,reverse=false,unavailable='بيانات الرحلة غير متاحة'}){if(!trip)return <div className="journey-block"><strong>{title}</strong><span>{unavailable}</span></div>;const from=reverse?(trip.to_city||trip.destination):(trip.from_city||trip.origin),to=reverse?(trip.from_city||trip.origin):(trip.to_city||trip.destination);return <div className="journey-block"><div className="journey-title"><BusFront size={17}/><strong>{title}</strong><span>{trip.trip_code||''}</span></div><div className="journey-route"><b>{from||'—'}</b><span>←</span><b>{to||'—'}</b></div><div className="journey-meta"><span>{weekdayAr(date)} · {date||'—'} {time||''}</span>{boardingPoint&&<span>{boardingPoint}{boardingTime?` · ${boardingTime}`:''}</span>}</div></div>}
