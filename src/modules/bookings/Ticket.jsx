...
export default function TicketPage({bookingNo,go}){
 ...
 const grossPaid=Number(b.paid_amount||0),refunded=Number(refundSummary.refunded||0),paid=Math.max(0,grossPaid-refunded),total=Number(b.total_price||0),remaining=Math.max(0,total-paid),refundDue=Math.max(0,paid-total);
 ...
 const paymentLabel=refundDue>0?'رصيد مستحق للعميل':remaining>0?'مدفوع جزئي':'مدفوع بالكامل';
 ...
 const wa=()=>{const msg=[`الماهر الماسي للسفر والسياحة`,`تذكرة سفر: ${b.booking_number}`,`العميل: ${b.customer_name||''}`,`نوع الرحلة: ${journeyLabel(b.journey_mode)||'—'}`,showOutbound&&outboundRouteLabel?`الذهاب: ${outboundRouteLabel}`:'',showOutbound&&trip?.departure_date?`تاريخ الذهاب: ${weekdayAr(trip.departure_date)} ${trip.departure_date} ${trip.departure_time||''}`:'',showReturn&&returnRouteLabel?`العودة: ${returnRouteLabel}`:'',showReturn&&returnDate?`تاريخ العودة: ${weekdayAr(returnDate)} ${returnDate} ${returnTime||''}`:'',`السكن: ${housingTypeLabel}`,housingType==='private'&&privateRoomPlanLabel?`تفاصيل الغرف: ${privateRoomPlanLabel}`:'',`المدفوع الصافي: ${money(paid)}`,refunded>0?`تم استرداده: ${money(refunded)}`:'',refundDue>0?`رصيد مستحق للعميل: ${money(refundDue)}`:remaining?`المتبقي: ${money(remaining)}`:'الحجز مسدد بالكامل'].filter(Boolean).join('\n');window.open(`https://wa.me/${phoneWa(b.customer_phone)}?text=${encodeURIComponent(msg)}`,'_blank')};
 ...
 return <>
  ...
  <article className={`ticket-page ${dense} ${ticketCancelled?'ticket-cancelled':''}`} dir={dir} lang={printLanguage}>
   ...
   <section className="ticket-section ticket-finance-section"><div className="ticket-section-heading"><h3>البيانات المالية</h3><span>{paymentLabel}</span></div><div className="ticket-finance-grid"><div><span>الإجمالي</span><b>{money(total)}</b></div><div><span>المدفوع الصافي</span><b>{money(paid)}</b></div>{refunded>0&&<div><span>تم استرداده</span><b>{money(refunded)}</b></div>}<div><span>{refundDue>0?'رصيد مستحق للعميل':'المتبقي'}</span><b>{money(refundDue>0?refundDue:remaining)}</b></div><div><span>عدد المسافرين</span><b>{passengers.length}</b></div></div></section>
