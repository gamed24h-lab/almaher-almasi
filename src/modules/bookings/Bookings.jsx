...
export default function Bookings({go,query='',branchParam='',financialParam='',statusParam='',datePresetParam='',fromDateParam='',toDateParam='',remainingOpParam='',remainingParam='',remainingMinParam='',remainingMaxParam='',limitParam='',sortParam=''}){
 ...
 useEffect(()=>{if(query)setQ(query);if(branchParam)setBranchId(branchParam);if(financialParam)setFinancial(financialParam);if(statusParam)setStatus(statusParam);if(datePresetParam)setDatePreset(datePresetParam);if(fromDateParam)setFromDate(fromDateParam);if(toDateParam)setToDate(toDateParam);if(remainingParam||remainingMinParam||remainingMaxParam)setBookingRules(r=>[...r.filter(x=>x.field!=='remaining'),...(remainingParam?[{field:'remaining',operator:remainingOpParam||'gt',value:remainingParam}]:[]),...(remainingMinParam?[{field:'remaining',operator:'gte',value:remainingMinParam}]:[]),...(remainingMaxParam?[{field:'remaining',operator:'lte',value:remainingMaxParam}]:[])]);if(sortParam)setSort(sortParam)},[query,branchParam,financialParam,statusParam,datePresetParam,fromDateParam,toDateParam,remainingOpParam,remainingParam,remainingMinParam,remainingMaxParam,sortParam]);
 ...
 const rows=useMemo(()=>{
   ...
   return limitParam?out.slice(0,Math.max(1,Number(limitParam)||25)):out;
 },[data.bookings,q,status,tripId,branchId,financial,sort,datePreset,fromDate,toDate,tripMap,passengerMap,refundSummary,bookingRules,bookingRuleFields,bookingRuleMode,limitParam]);
