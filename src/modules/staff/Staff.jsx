...
const GENERAL_PERMS=[
 ['branchBooking','إنشاء حجوزات'],['viewBookings','عرض الحجوزات'],['editBookings','تعديل الحجوزات'],['editPassenger','تعديل الركاب'],['changeTrip','نقل الحجز بين الرحلات'],['crossBranchReturn','اختيار عودة من فرع آخر'],['printTickets','طباعة التذاكر'],
...
const HIGH_RISK=new Set(['managePermissions','manageUsers','allBranches','allBranchesFinance','crossBranchReturn','addBranches','manageCompanyProfile','refund_approve','refund_complete','approvals','deleteDestinations','manageDestinations','deleteDrivers','deleteVehicles','manageDrivers','deleteHotels','manageHotels']);
...
const ROLE_TEMPLATES={
 'موظف حجوزات':['branchBooking','viewBookings','editBookings','editPassenger','changeTrip','printTickets','viewDestinations','customers','documents'],
 'محاسب':['viewBookings','finance','payments','viewPaymentReceipts','printPaymentReceipts','reconcileFinance','expenses','shifts','refunds','refund_request','reports','printReports'],
 'مشرف تشغيل':['viewBookings','trips','viewDestinations','addDestinations','editDestinations','manageDestinationRoutes','operations','manifest','housingManifest','scanner','housing','viewHotels','addHotels','editHotels','linkHotels','manageHotelRooms','seats','fleet','viewFleet','vehicles','addVehicles','editVehicles','viewDrivers','addDrivers','editDrivers','assignFleet','manageMaintenance','returns','notifications'],
 'موظف تسكين':['viewBookings','editPassenger','housing','housingManifest','viewHotels','linkHotels','manageHotelRooms','documents','printReports'],
 'خدمة عملاء':['viewBookings','customers','crm','notifications','documents','printTickets'],
 'مدير فرع':['branchBooking','viewBookings','editBookings','editPassenger','changeTrip','printTickets','trips','viewDestinations','addDestinations','editDestinations','manageDestinationRoutes','operations','manifest','housingManifest','scanner','housing','viewHotels','addHotels','editHotels','linkHotels','manageHotelRooms','seats','fleet','viewFleet','vehicles','addVehicles','editVehicles','viewDrivers','addDrivers','editDrivers','assignFleet','manageMaintenance','returns','finance','payments','viewPaymentReceipts','printPaymentReceipts','reconcileFinance','expenses','shifts','refunds','refund_request','approvals','crm','customers','documents','notifications','reports','printReports','manageUsers','viewBranches','editBranches'],
...
export default function Staff(){
 ...
 const canApprovePermissionChanges=isDeveloper||user?.role==='مدير عام'||user?.permissions?.all||!!user?.permissions?.managePermissions||!!user?.permissions?.approvals;
 ...
 const pendingPermissionApprovals=approvals.filter(x=>x.request_type==='staff_permission_change'&&x.status==='pending');
 ...
 function approvalLabels(r){
   const changes=r.request_payload?.changes||{},target=userMap[String(r.entity_id)]||{},before=target.permissions||{},after=changes.permissions||{};const labels=[];
   for(const k of HIGH_RISK)if(!!before[k]!==!!after[k])labels.push(permLabel(k));
   if(changes.role&&changes.role!==target.role)labels.push(`الدور: ${target.role||'—'} ← ${changes.role}`);
   if(changes.account_mode&&changes.account_mode!==modeOf(target))labels.push(changes.account_mode==='production'?'تحويل إلى تشغيل فعلي':'تحويل إلى تدريب');
   return labels.length?labels:['تغيير صلاحيات حساس'];
 }
 const cols=[
 ...
 const approvalCols=[
  ...
  {key:'risk',label:'التغيير الحساس',render:r=><div className="finance-actions">{approvalLabels(r).map(x=><Badge key={x} tone="orange">{x}</Badge>)}</div>},
  {key:'requested_by',label:'طالب التغيير',render:r=>r.requested_by||'—'},
 ...
 <Modal open={open} onClose={()=>setOpen(false)} title={form.id?'تعديل الموظف':'إضافة موظف'} wide><form onSubmit={save} className="form-grid">
  ...
  {!isDeveloper&&canEditPermissions&&form.id&&<div className="permissions-box" style={{gridColumn:'1/-1'}}><div className="card-title"><h3><KeyRound size={18}/> إعادة التحقق للعمليات الحساسة</h3><small>عند رفع الدور، تحويل الحساب إلى Production أو تغيير صلاحية عليها 🔐، أدخل كلمة مرورك الحالية. سيُنشئ الخادم طلب موافقة ثانية بدل التطبيق المباشر.</small></div><Field label="كلمة مرورك الحالية"><Input type="password" value={form.reauthPassword||''} onChange={e=>setForm(x=>({...x,reauthPassword:e.target.value}))} autoComplete="current-password"/></Field></div>}
