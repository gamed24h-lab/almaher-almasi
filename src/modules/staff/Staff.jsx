...
const GENERAL_PERMS=[
 ...
 ['crm','CRM'],['customers','العملاء'],['documents','المستندات'],['notifications','الإشعارات'],['automation','الأتمتة'],['reports','التقارير'],['printReports','طباعة التقارير'],['manageUsers','إدارة بيانات الموظفين'],['managePermissions','إدارة صلاحيات الموظفين'],['allBranches','تشغيل كل الفروع'],['allBranchesFinance','مالية كل الفروع']
];
...
export default function Staff(){
 ...
 const branches=data.branches||[],users=data.users||[];const isDeveloper=String(user?.role||'').toLowerCase()==='developer';const actorRank=rankOf(user?.role);const canEditPermissions=isDeveloper||user?.role==='مدير عام'||user?.permissions?.all||!!user?.permissions?.managePermissions;
 ...
 <Modal open={open} onClose={()=>setOpen(false)} title={form.id?'تعديل الموظف':'إضافة موظف'} wide><form onSubmit={save} className="form-grid">
  ...
  <Field label="الدور / قالب الصلاحيات"><Select disabled={!canEditPermissions&&!!form.id} value={form.role||allowedRoles[0]||'موظف'} onChange={e=>{const role=e.target.value;setForm(x=>({...x,role}));}}>{(canEditPermissions?allowedRoles:[form.role||'موظف']).map(r=><option key={r}>{r}</option>)}</Select>{canEditPermissions&&<Button type="button" onClick={()=>applyTemplate()}><Wand2 size={14}/> تطبيق قالب هذا الدور</Button>}</Field>
  ...
  {canEditPermissions?<div className="permissions-box"><div className="card-title"><h3><ShieldCheck size={18}/> الصلاحيات التشغيلية</h3><small>تظهر لك فقط الصلاحيات التي يحق لك تفويضها، ولا يمكنك منح صلاحية لا تملكها.</small></div><div className="permission-grid">{visibleGeneralPerms.map(([k,l])=><label className="check permission-check" key={k}><input type="checkbox" checked={!!form.permissions?.[k]} onChange={e=>setForm(x=>({...x,permissions:{...(x.permissions||{}),[k]:e.target.checked}}))}/><span>{l}</span></label>)}</div></div>:<div className="permissions-box"><div className="card-title"><h3><ShieldCheck size={18}/> الصلاحيات</h3><small>لديك إدارة بيانات الموظفين فقط. الصلاحيات والدور ووضع التشغيل محفوظة كما هي ولا يمكن تعديلها من هذا الحساب.</small></div></div>}
