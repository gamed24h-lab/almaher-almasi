...
export default function IDStudio(){
  ...
  return <div className="idstudio-page">
    ...
    <div className="idstudio-grid">
      <Card className="idstudio-control"><div className="idstudio-section-title"><div><h3>إصدار بطاقة جديدة</h3><small>رقم البطاقة وQR Token يُولدان من الخادم تلقائيًا.</small></div></div><form onSubmit={createCard} className="idstudio-form">
      ...
      <Card className="idstudio-preview-card"><div className="idstudio-section-title"><div><h3>المعاينة الحقيقية</h3><small>{selectedCard?`${selectedCard.card_number} · ${statusAr(selectedCard.status)}`:`${selected?.name_ar||'القالب'} · معاينة قبل الحفظ`}</small></div><Badge>CR80</Badge></div>
        {selectedCard?.print_mode==='front_back'&&<div className="idstudio-segment"><button className={previewSide==='front'?'active':''} onClick={()=>setPreviewSide('front')}>الوجه الأمامي</button><button className={previewSide==='back'?'active':''} onClick={()=>setPreviewSide('back')}>الوجه الخلفي</button></div>}
        <div className="idstudio-real-preview"><IDCardFace card={previewCard} side={previewSide}/></div>
        <div className="idstudio-safe">85.60 × 53.98 مم · QR الحقيقي يظهر بعد حفظ البطاقة. البطاقات غير المعتمدة تحمل علامة «غير معتمدة».</div>
        ...
        <div className="idstudio-templates">{templates.map(t=><button type="button" key={t.code} className={`idstudio-template ${form.template_code===t.code?'selected':''}`} onClick={()=>{setSelectedCard(null);setField('template_code',t.code)}}><div className={`template-thumb ${TEMPLATE_KIND[t.code]||'minimal'}`}><i></i><b></b><em></em></div><strong>{t.name_ar}</strong><span>{t.name_en||''}</span></button>)}</div>{canManageTemplates&&<div className="idstudio-note">لديك صلاحية إدارة القوالب. نشر إصدار قالب جديد سيتم فصله عن البطاقات التاريخية.</div>}
      </Card>
    </div>
