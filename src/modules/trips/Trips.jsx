...
import {Plus,RefreshCw,Pencil,UsersRound,XCircle,CalendarDays} from 'lucide-react';
...
export default function Trips({go}){
 ...
 const [scheduleMode,setScheduleMode]=useState('single'),[rangeEnd,setRangeEnd]=useState(''),[weekdays,setWeekdays]=useState([]);
 ...
 const tripColumns=[
   ...
   {key:'edit',label:'',render:r=>has(user,'trips')?<div className="row-actions"><Button onClick={e=>{e.stopPropagation();openEdit(r)}}><Pencil size={14}/> تعديل</Button>{String(r.status).toLowerCase()!=='cancelled'&&<Button onClick={async e=>{e.stopPropagation();const reason=prompt('سبب إلغاء الرحلة:','');if(reason===null)return;await api.admin({action:'sync_trips',rows:[{...r,status:'cancelled',operational_notes:`${r.operational_notes||''}\n[إلغاء] ${reason||'بدون سبب'}`}]});await refresh()}}><XCircle size={14}/> إلغاء</Button>}</div>:null}
 ];
 return <>
  ...
  <Modal open={open} onClose={()=>setOpen(false)} title={editing?`تعديل الرحلة ${editing.trip_code}`:'إنشاء رحلة جديدة'} wide>
   <form className="form-grid" onSubmit={save} key={editing?.id||'new'}>
    ...
    <datalist id="almaher-cities">{cities.map(c=><option key={c} value={c}/>)}</datalist><Field label="من"><Input list="almaher-cities" name="from_city" required defaultValue={editing?.from_city||editing?.origin||''} placeholder="اختر أو اكتب مدينة"/></Field>
    <Field label="إلى"><Input list="almaher-cities" name="to_city" required defaultValue={editing?.to_city||editing?.destination||''} placeholder="اختر أو اكتب مدينة"/></Field>
