...
export default function Wallets(){
 ...
 return <div>
  ...
  <div style={{display:'grid',gridTemplateColumns:'minmax(0,1fr) minmax(0,1.2fr)',gap:14,marginTop:14}} className="wallet-layout">
   <Card><div className="card-title"><div><h3>العملاء</h3><small>اضغط على العميل لعرض كشف المحفظة.</small></div></div><div style={{display:'grid',gap:8}}>{rows.length?rows.map(w=><button key={w.id} type="button" onClick={()=>openWallet(w)} style={{border:'1px solid #dfe5ec',background:String(selected?.id)===String(w.id)?'#eef4fb':'#fff',borderRadius:12,padding:12,textAlign:'start',cursor:'pointer'}}><div style={{display:'flex',justifyContent:'space-between',gap:10,alignItems:'center'}}><strong>{w.customer_name||'عميل بدون اسم'}</strong><b>{money(w.balance||0)}</b></div><div className="muted-small" style={{marginTop:5}}>{w.customer_phone||'—'} · {w.customer_identity||'—'}</div></button>):<div className="muted-small">لا توجد محافظ مطابقة.</div>}</div></Card>
   <Card><div className="card-title"><div><h3>كشف المحفظة</h3><small>{selected?`${selected.customer_name||''} · ${selected.customer_identity||''}`:'اختر عميلًا أولًا'}</small></div>{details&&<Badge>{money(details.balance||0)}</Badge>}</div>{details?<div><div style={{display:'grid',gridTemplateColumns:'repeat(2,minmax(0,1fr))',gap:10,marginBottom:12}}><div><span className="muted-small">الرصيد الحالي</span><div style={{fontSize:24,fontWeight:900}}>{money(details.balance||0)}</div></div><div><span className="muted-small">آخر تحديث</span><div>{details.wallet?.updated_at?new Date(details.wallet.updated_at).toLocaleString('ar-SA'):'—'}</div></div></div><div style={{display:'grid',gap:8}}>{(details.transactions||[]).length?details.transactions.map(tx=><div key={tx.id} style={{border:'1px solid #e4e9ef',borderRadius:12,padding:10}}><div style={{display:'flex',justifyContent:'space-between',gap:10}}><strong><ReceiptText size={15} style={{verticalAlign:'middle',marginInlineEnd:5}}/>{txLabel(tx.transaction_type)}</strong><b>{tx.transaction_type==='debit'?'-':'+'}{money(tx.amount)}</b></div><div className="muted-small" style={{marginTop:5}}>{tx.reason||'—'} · {statusLabel(tx.status)}</div><div className="muted-small">{tx.reference_no||'بدون مرجع'} · {tx.created_at?new Date(tx.created_at).toLocaleString('ar-SA'):'—'}</div></div>):<div className="muted-small">لا توجد حركات على هذه المحفظة.</div>}</div></div>:<div className="muted-small">اختر محفظة من القائمة لعرض التفاصيل.</div>}</Card>
  </div>
 </div>
}
