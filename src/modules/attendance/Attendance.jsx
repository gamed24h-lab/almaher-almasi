...
export default function Attendance(){
 ...
 return <>
  ...
  {activeTab==='overview'&&<>
   ...
   <Card><div className="card-title"><div><h3>آخر الحركات</h3><small>آخر 10 بصمات مستلمة للمتابعة السريعة</small></div><Badge>{Math.min(10,logs.length)}</Badge></div><Table rows={logs.slice(0,10)} columns={logCols}/></Card>
  </>}

  {activeTab==='employees'&&<AttendanceEmployees state={state} onChanged={load} onError={setError} onNotice={setNotice}/>}
  {activeTab==='devices'&&<><AttendanceDeviceData state={state} onChanged={load} onError={setError} onNotice={setNotice}/><Card><div className="card-title"><h3>أجهزة البصمة</h3><Badge>{devices.length}</Badge></div><Table rows={devices} columns={deviceCols}/></Card></>}
  {activeTab==='links'&&<><Card><div className="card-title"><h3>ربط أرقام الأجهزة بالموظفين</h3><Badge>{links.length}</Badge></div><Table rows={links} columns={linkCols}/></Card><Card><div className="card-title"><h3>الحركات المستلمة</h3><Badge>{logs.length}</Badge></div><Table rows={logs} columns={logCols}/></Card></>}
