...
export default function Attendance(){
 ...
 const deviceCols=[
  ...
  {key:'connection',label:'الاتصال',render:r=>online(r)?<Badge tone="green"><Wifi size={13}/> متصل</Badge>:<Badge tone="red"><WifiOff size={13}/> غير متصل</Badge>},
  {key:'seen',label:'آخر اتصال',render:r=>fmtDate(r.last_seen_at)},
 ...
 <Modal open={linkOpen} onClose={()=>setLinkOpen(false)} title="ربط موظف بجهاز البصمة" wide><form onSubmit={saveLink} className="form-grid">
  ...
  <Field label="حساب الموظف بالنظام"><Select value={linkForm.staff_user_id||''} onChange={e=>{const u=userMap.get(String(e.target.value));setLinkForm(x=>({...x,staff_user_id:e.target.value,display_name:u?.name||x.display_name}))}}><option value="">بدون حساب نظام / اسم يدوي</option>{users.map(u=><option key={u.id} value={u.id}>{u.name} — {branchMap.get(String(u.branch_id))||'إدارة عامة'}</option>)}</Select></Field>
  <Field label="الاسم اليدوي"><Input value={linkForm.display_name||''} onChange={e=>setLinkForm(x=>({...x,display_name:e.target.value}))} placeholder="يستخدم إذا لم يكن له حساب بالنظام"/></Field>
