...
export default function AttendanceBiometricReconciliation({state,onChanged,onError,onNotice,onOpenLinks,onOpenDevices}){
 ...
 const issues=useMemo(()=>{
  ...
  for(const inv of inventory){const dk=String(inv.device_id),da=inventoryByDevice.get(dk)||[];da.push(inv);inventoryByDevice.set(dk,da);const pk=dk+'|'+String(inv.device_pin),pa=inventoryByDevicePin.get(pk)||[];pa.push(inv);inventoryByDevicePin.set(pk,pa)}
  ...
  for(const d of devices.filter(x=>x.status==='active')){
   ...
   for(const inv of deviceInventory){
    const pin=String(inv.device_pin),matches=linkByDevicePin.get(did+'|'+pin)||[];
    if(!matches.some(x=>x.attendance_employee_id)&&!reportedUnlinkedPins.has(pin)){
     const bios=inventoryByDevicePin.get(did+'|'+pin)||[];reportedUnlinkedPins.add(pin);
     out.push({id:'unlinked-bio:'+did+':'+pin,type:'unlinked_biometric',severity:'warning',device_id:d.id,branch_id:d.branch_id,device_pin:pin,title:'بصمة موجودة لـ PIN غير مربوط',detail:'تم اكتشاف '+bios.length+' قالب بصمة على PIN '+pin+' لكن لا يوجد له موظف حضور مربوط.',action:'links'});
    }
   }
  }
