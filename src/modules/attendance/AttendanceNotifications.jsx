  ...
  <Modal open={rulesOpen} onClose={()=>setRulesOpen(false)} title="قواعد تصعيد تنبيهات الحضور" wide>
   <div style={{display:'grid',gap:14}}>
    ...
    <Card><div className="card-title"><h3>{ruleForm.id?'تعديل قاعدة التصعيد':'قاعدة تصعيد جديدة'}</h3></div><form onSubmit={saveRule} className="form-grid">
     ...
     <Field label="نوع التنبيه"><Select value={ruleForm.category} onChange={e=>setRuleForm(x=>({...x,category:e.target.value}))}><option value="*">كل الأنواع</option><option value="device_health">صحة الجهاز</option><option value="predictive">استباقي</option><option value="linking">ربط الحركات</option><option value="maintenance">صيانة وقائية</option><option value="attendance_workflow">متابعة الحضور والموافقات</option></Select></Field>
