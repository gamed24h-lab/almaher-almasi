...
function clockSyncLabel(v,pending=false){if(pending&&['queued','accepted','accepted_unverified'].includes(v))return 'تمت المزامنة — بانتظار أول بصمة للتحقق';return v==='queued'?'بانتظار الجهاز':v==='accepted'?'استلم الجهاز التحديث':v==='accepted_unverified'?'استلم التحديث — جارٍ التحقق':v==='verified'?'تمت المزامنة والتحقق':v==='verification_failed'?'فشل التحقق — الساعة ما زالت غير مضبوطة':v==='failed'?'فشل إرسال المزامنة':v||'لم تُنفذ بعد'}
...
export default function AttendanceDeviceData({state,onChanged,onError,onNotice,onOpenLinks}){
 ...
 const cols=[
  ...
  {key:'clock',label:'ساعة الجهاز',render:d=>{const h=healthByDevice.get(String(d.id)),sec=Number(h?.clock_drift_seconds),known=h?.clock_confirmed===true&&Number.isFinite(sec),pending=h?.clock_verification_pending===true,abs=Math.abs(sec||0),label=pending?'تم التحديث — بانتظار التحقق':!known?'غير مؤكدة':abs<=120?'مضبوطة':sec>0?'متقدمة '+(abs>=3600?(abs/3600).toFixed(abs%3600<120?0:1)+' س':Math.round(abs/60)+' د'):'متأخرة '+(abs>=3600?(abs/3600).toFixed(abs%3600<120?0:1)+' س':Math.round(abs/60)+' د'),tone=pending?'blue':!known?'gray':abs<=120?'green':abs>900?'red':'orange',sync=d?.metadata?.last_clock_sync;return <div><Badge tone={tone}>{label}</Badge><div className="muted-small" style={{marginTop:3}}>{pending?'الفرق السابق '+(Number.isFinite(sec)?Math.round(sec/60)+' دقيقة':'—')+' — لا يُعتبر خللًا حاليًا حتى وصول بصمة Live جديدة':'آخر فحص: '+fmt(h?.clock_checked_at)}</div>{sync?.status&&<div className="muted-small">المزامنة: {clockSyncLabel(sync.status,sync.awaiting_verification===true)}</div>}{sync?.verified_at&&<div className="muted-small">تحقق: {fmt(sync.verified_at)}</div>}</div>}},
  ...
  <div style={{display:'grid',gap:14}}>
   ...
   <Card><div className="card-title"><div><h3><Activity size={18}/> نتيجة التشخيص</h3><small>الاختبار يرسل INFO فقط ولا يعدّل الموظفين أو سجل الحضور.</small></div><Badge tone={diagCommand?.status==='success'?'green':diagCommand?.status==='failed'?'red':'orange'}>{diagCommand?cmdLabel(diagCommand.status):'لم يبدأ'}</Badge></div>
    <div className="form-grid">
     ...
     <Field label="حالة ساعة الجهاز"><Input readOnly value={diagHealth?.clock_verification_pending?'تم التحديث — بانتظار التحقق':diagHealth?.clock_confirmed?(Math.abs(Number(diagHealth.clock_drift_seconds)||0)<=120?'مضبوطة':Number(diagHealth.clock_drift_seconds)>0?'متقدمة':'متأخرة'):'غير مؤكدة'}/></Field>
     <Field label={diagHealth?.clock_verification_pending?'فرق الساعة السابق بالدقائق':'فرق الساعة بالدقائق'}><Input dir="ltr" readOnly value={diagHealth?.clock_confirmed?String(diagHealth.clock_drift_minutes):'—'}/></Field>
     ...
     <Field label="حالة آخر مزامنة"><Input readOnly value={clockSyncLabel(diagDevice?.metadata?.last_clock_sync?.status,diagDevice?.metadata?.last_clock_sync?.awaiting_verification===true)}/></Field>
