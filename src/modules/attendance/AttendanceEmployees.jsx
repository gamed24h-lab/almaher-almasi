...
export default function AttendanceEmployees({state,onChanged,onError,onNotice}){
 const branches=state.branches||[],employees=state.employees||[],users=state.users||[],links=state.links||[],devices=state.devices||[],shiftPeriods=state.shiftPeriods||[],deviceShiftTemplates=state.deviceShiftTemplates||[];
 ...
 const cols=[
  ...
  {key:'edit',label:'',render:r=><div className="finance-actions">{state.permissions?.manage_employees&&<Button onClick={()=>edit(r)}>تعديل</Button>}{state.permissions?.manage_devices&&(linksMap.get(String(r.id))||[]).length>0&&<Button onClick={()=>push(r)} disabled={pushBusy===r.id}><RefreshCw size={14}/>{pushBusy===r.id?' جاري الرفع...':' إعادة رفع'}</Button>}</div>}
 ];

 return <><Card><div className="card-title"><div><h3><Users size={19}/> موظفو الحضور</h3><small>اختر فترات جاهزة من إعدادات الأجهزة أو أضف فترات مخصصة مختلفة لكل موظف. الحفظ يجهّز رفع بيانات الموظف تلقائيًا للأجهزة المرتبطة.</small></div><div className="finance-actions"><Badge>{employees.length}</Badge>{state.permissions?.manage_employees&&<Button variant="primary" onClick={add}><Plus size={15}/> موظف جديد</Button>}</div></div><Table rows={employees} columns={cols}/></Card>
 ...
 <Modal open={open} onClose={()=>setOpen(false)} title={form.id?'تعديل موظف حضور':'إضافة موظف حضور'} wide><form onSubmit={save} className="form-grid">
  ...
  <div className="field" style={{gridColumn:'1/-1'}}>
   <span>فترات الجهاز الجاهزة</span>
   {availableTemplates.length?<div style={{display:'grid',gap:8}}>
    {availableTemplates.map(t=>{const selected=(form.shift_periods||[]).some(p=>String(p.device_shift_template_id)===String(t.id));return <div key={t.id} className="success-note" style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:10}}>
      <div><strong>{deviceMap.get(String(t.device_id))?.name||'جهاز'} — {t.name}</strong><div className="muted-small">{String(t.start_time).slice(0,5)} — {String(t.end_time).slice(0,5)} · سماح {t.grace_minutes} د</div></div>
      <Button type="button" disabled={selected} onClick={()=>addTemplatePeriod(t)}>{selected?'مضافة':'إضافة للموظف'}</Button>
     </div>})}
   </div>:<div className="muted-small">لا توجد فترات جاهزة في أجهزة هذا الفرع. يمكنك إضافة فترة مخصصة أو تعريف الفترات من إعدادات الجهاز.</div>}
  </div>
   ...
   <div style={{display:'grid',gap:10}}>
    {(form.shift_periods||[]).map((p,i)=><Card key={i}><div className="form-grid">
      <div style={{gridColumn:'1/-1',display:'flex',justifyContent:'space-between',alignItems:'center',gap:8}}>
       <div>{p.device_shift_template_id?<Badge tone="green">فترة من الجهاز: {templateMap.get(String(p.device_shift_template_id))?.name||p.label}</Badge>:<Badge>فترة مخصصة</Badge>}</div>
      ...
      {p.device_shift_template_id&&<div className="muted-small" style={{gridColumn:'1/-1'}}>لو عدلت الوقت أو الاسم هنا، الفترة تتحول تلقائيًا إلى «مخصصة» لهذا الموظف ولن تتأثر لاحقًا بتعديل فترة الجهاز.</div>}
    </div></Card>)}
