...
export default function AttendanceReports({state,onError,onNotice}){
 ...
 <div className="form-grid">
  ...
  <Field label="الموظف"><Select value={filters.attendance_employee_id} onChange={e=>setFilters(x=>({...x,attendance_employee_id:e.target.value}))}><option value="">كل الموظفين</option>{reportEmployees.map(x=><option key={x.id} value={x.id}>{x.employee_code} — {x.name}</option>)}</Select></Field>
  <Field label="الجهاز"><Select value={filters.device_id} onChange={e=>setFilters(x=>({...x,device_id:e.target.value}))}><option value="">كل الأجهزة</option>{reportDevices.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</Select></Field>
 </div>
