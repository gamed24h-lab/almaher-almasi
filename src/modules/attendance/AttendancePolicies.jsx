...
export default function AttendancePolicies({state,onChanged,onError,onNotice}){
 ...
 <form onSubmit={save} className="form-grid">
  ...
  {!!branchVersions.length&&<div className="field" style={{gridColumn:'1/-1'}}><span>سجل سريان السياسة</span><div style={{display:'grid',gap:6}}>{branchVersions.map(v=>{const s=v.policy_snapshot||{},mode=s.attendance_mode==='mobile'?'جوال':s.attendance_mode==='hybrid'?'مختلط':'بصمة',today=defaults.policy_effective_from,isFuture=String(v.effective_from)>today,isCurrent=String(v.effective_from)<=today&&(!v.effective_to||String(v.effective_to)>=today);return <div key={v.id} className="success-note" style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:10,flexWrap:'wrap'}}><div><strong>{v.effective_from}{v.effective_to?' → '+v.effective_to:' → مستمر'}</strong> · {mode} {isFuture?<Badge tone="orange">مجدولة</Badge>:isCurrent?<Badge tone="green">السارية الآن</Badge>:<Badge>تاريخية</Badge>}{v.reason?<div className="muted-small">{v.reason}</div>:null}</div><Button type="button" onClick={()=>setForm(x=>({...x,...s,branch_id:branchId,data_environment:state.scope?.environment||'training',policy_effective_from:v.effective_from,reason:v.reason||''}))}>تحميل للتعديل</Button></div>})}</div></div>}
