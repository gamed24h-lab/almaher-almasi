...
export default function Shell({children,route,go}){
 ...
 const labelFor=x=>x.key==='destinations'?'إدارة الوجهات':x.key==='wallets'?'محافظ العملاء':x.key==='attendance'?'الحضور والبصمة':x.key==='idStudio'?'بطاقات ID':x.key==='workflow'?'مركز الإجراءات':t(x.key);
