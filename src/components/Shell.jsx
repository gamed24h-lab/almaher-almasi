...
const items=[
 ...
 {p:'/forms',key:'formsCenter',Icon:FileText,perms:['forms_view','forms_submit','forms_approve','forms_manage_templates'],group:'management'},
 ...
 {p:'/audit',key:'auditCenter',Icon:Activity,perms:['auditLog','managePermissions','editBookings','editPassenger'],group:'management'},
...
export default function Shell({children,route,go}){
 ...
 const [collapsed,setCollapsed]=useState(false),[q,setQ]=useState(''),[focus,setFocus]=useState(false),[mobileMenu,setMobileMenu]=useState(false),[commandOpen,setCommandOpen]=useState(false);
 ...
 const labelFor=x=>x.key==='agentCollections'?'تحصيل الوكلاء':x.key==='destinations'?'إدارة الوجهات':x.key==='wallets'?'محافظ العملاء':x.key==='attendance'?'الحضور والبصمة':x.key==='idStudio'?'بطاقات ID':x.key==='workflow'?'مركز الإجراءات':x.key==='formsCenter'?'النماذج والموافقات':x.key==='qualityCenter'?'جودة البيانات':x.key==='auditCenter'?'السجل والتكرارات':t(x.key);
