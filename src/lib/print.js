...
function singlePageFitScript(){
 return `<script>(function(){function mm(v){var d=document.createElement('div');d.style.cssText='position:absolute;visibility:hidden;width:'+v+'mm;height:1mm';document.body.appendChild(d);var n=d.getBoundingClientRect().width;d.remove();return n}function fit(){var root=document.querySelector('.isolated-print-root');var ticket=document.querySelector('.ticket-page');if(!root||!ticket)return;ticket.style.transform='';root.style.height='';ticket.style.width='190mm';ticket.style.maxWidth='190mm';ticket.style.margin='0 auto';var maxH=mm(277);var r=ticket.getBoundingClientRect();var h=Math.max(ticket.scrollHeight,r.height);var scale=Math.min(1,maxH/h);if(scale<0.999){ticket.style.transformOrigin='top center';ticket.style.transform='scale('+scale+')';root.style.height=Math.ceil(h*scale)+'px';root.style.overflow='hidden'}}function ready(){var imgs=[].slice.call(document.images||[]);Promise.all(imgs.map(function(i){return i.complete?Promise.resolve():new Promise(function(res){i.addEventListener('load',res,{once:true});i.addEventListener('error',res,{once:true})})})).then(function(){requestAnimationFrame(function(){requestAnimationFrame(fit)})})}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',ready,{once:true});else ready();window.addEventListener('beforeprint',fit)})();</script>`;
}
export function printElement(element,{title='Al Maher',dir,lang,pageSize='A4',orientation='portrait',bodyAttributes={},singlePage=false,purpose='print'}={}){
 ...
 const contentWidth=normalized==='80mm'?'74mm':normalized==='58mm'?'54mm':'';
 ...
 const thermalCss=thermal?`html,body{width:100%!important;max-width:none!important}body{padding:0!important;overflow-x:hidden!important}.isolated-print-root{width:${contentWidth}!important;max-width:${contentWidth}!important;margin:0 auto!important}body[data-print-mode="80"] .ticket-page,body[data-print-mode="58"] .ticket-page{width:100%!important;max-width:100%!important;margin:0 auto!important}img,svg,table{max-width:100%!important}table{width:100%!important;table-layout:fixed!important}th,td{overflow-wrap:anywhere!important;word-break:break-word!important}`:'';
 const singleCss=singlePage&&!thermal?`.isolated-print-root{width:190mm!important;max-width:190mm!important;min-height:0!important;height:auto!important;margin:0 auto!important;overflow:visible!important}.ticket-page{width:190mm!important;max-width:190mm!important;min-width:190mm!important;break-inside:avoid!important;page-break-inside:avoid!important;page-break-after:avoid!important;margin:0 auto!important;box-shadow:none!important}`:'';
 const pdfCss=purpose==='pdf'?`body:before{content:'PDF';display:none}`:'';
 const shellCss=`<style>@page{size:${page};margin:${thermal?'0':'10mm'}}*{box-sizing:border-box}html,body{background:#fff!important;margin:0!important;padding:0!important}body{direction:${actualDir};-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}.isolated-print-root{width:100%;max-width:none!important;margin:0!important;box-shadow:none!important}.isolated-print-root .print-only{display:block!important}.print-field-value{display:inline-block;padding:4px 0}a{color:inherit;text-decoration:none}${thermalCss}${singleCss}${pdfCss}@media print{html,body{width:auto!important;min-width:0!important}.no-print,.page-actions,button{display:none!important}.ticket-page{break-inside:avoid!important;page-break-inside:avoid!important}}</style>`;
 const fit=singlePage&&!thermal?singlePageFitScript():'';
 const html=`<!doctype html><html lang="${esc(actualLang)}" dir="${esc(actualDir)}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=yes"><title>${esc(title)}</title>${cssAssets()}${shellCss}</head><body${attrs}><div class="isolated-print-root">${clone.outerHTML}</div>${fit}</body></html>`;
 printHtmlDocument(html,{title,delay:singlePage?700:450});
}
